...
auto main() -> int {
    return TestSuite("nimblecas.bigmatrix")
        ...
        .test("identity_zero_equality",
              [](TestContext& t) {
                  const auto id = BigMatrix::identity(3);
                  ...
                  const auto z = BigMatrix::zero(2, 3);
        ...
        .test("arithmetic_add_subtract_scale",
              [](TestContext& t) {
                  ...
                  const auto half = a.scale(brat(1, 2)).value();
        ...
        .test("multiply_and_transpose",
              [](TestContext& t) {
                  ...
                  const auto prod = bmat({{1, 2}, {3, 4}}).multiply(bmat({{5, 6}, {7, 8}})).value();
                  t.expect(prod == bmat({{19, 22}, {43, 50}}), "2x2 product matches hand result");

                  // transpose of a 2x3
                  const auto tr = bmat({{1, 2, 3}, {4, 5, 6}}).transpose().value();
        ...
        .test("determinant_multiplicativity_and_transpose",
              [](TestContext& t) {
                  ...
                  const auto da = a.determinant().value();
                  const auto db = b.determinant().value();
                  const auto dab = a.multiply(b).value().determinant().value();
        ...
        .test("determinant_rational_entries",
              [](TestContext& t) {
                  ...
                  const auto m = BigMatrix::from_rows({{brat(1, 2), brat(1, 3)},
                                                 {brat(1, 4), brat(1, 5)}})
                               .value();
                  ...
                  const auto m2 = BigMatrix::from_rows({{brat(2, 3), brat(1, 6)},
                                                  {brat(1, 2), brat(3, 4)}})
                                .value();
        ...
        .test("from_matrix_promotion_round_trip",
              [](TestContext& t) {
                  ...
                  const auto a = Matrix::from_rows({{Rational::from_int(3), Rational::make(1, 2).value()},
                  ...
                  const auto wide = BigMatrix::from_matrix(imat({{1, 2, 3}})).value();
