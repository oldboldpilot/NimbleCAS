    ...
    return TestSuite("nimblecas.lie")
        ...
        .test("antisymmetry",
              [](TestContext& t) {
                  auto a = mat({{1, 2}, {3, 4}});
                  auto b = mat({{0, -1}, {5, 2}});
                  const auto ab = lie_bracket(a, b).value();
                  const auto ba = lie_bracket(b, a).value();
                  // [A,B] = -[B,A]  <=>  [A,B] + [B,A] = 0.
        ...
        .test("bilinearity",
              [](TestContext& t) {
                  ...
                  const auto lhs_arg = a.scale(sa).value().add(b.scale(sb).value()).value();
                  const auto lhs = lie_bracket(lhs_arg, c).value();

                  const auto rhs = lie_bracket(a, c).value().scale(sa).value()
        ...
        .test("jacobi_identity",
              [](TestContext& t) {
                  ...
                  for (const auto& [a, b, c] : triples) {
                      const auto bc = lie_bracket(b, c).value();
                      const auto ca = lie_bracket(c, a).value();
                      const auto ab = lie_bracket(a, b).value();
                      const auto t1 = lie_bracket(a, bc).value();
                      const auto t2 = lie_bracket(b, ca).value();
                      const auto t3 = lie_bracket(c, ab).value();
                      const auto sum = t1.add(t2).value().add(t3).value();
        ...
        .test("structure_constants_levi_civita",
              [](TestContext& t) {
                  ...
                  const auto epsilon = [](std::size_t i, std::size_t j, std::size_t k) -> std::int64_t {
                      if (i == j || j == k || i == k) {
                          return 0;
                      }
        ...
        .test("structure_constants_not_closed",
              [](TestContext& t) {
                  ...
                  const std::vector<Matrix> partial{so3_x(), so3_y()};
        ...
        .test("adjoint_operator",
              [](TestContext& t) {
                  ...
                  const auto ad_x = adjoint_matrix(so3_x(), basis).value();
                  auto e1 = mat({{0}, {1}, {0}});
                  const auto image = ad_x.multiply(e1).value();
        ...
        .test("adjoint_action_equals_conjugation",
              [](TestContext& t) {
                  ...
                  auto l = mat({{0, 1}, {0, 0}});
                  auto f = mat({{1, 2}, {3, 4}});
                  const auto exp_l = Matrix::identity(2).add(l).value();          // I + L
                  const auto exp_neg = Matrix::identity(2).subtract(l).value();   // I - L
                  const auto conj = exp_l.multiply(f).value().multiply(exp_neg).value();
