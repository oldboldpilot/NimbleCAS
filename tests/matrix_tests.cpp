    ...
    return TestSuite("nimblecas.matrix")
        ...
        .test("multiply_and_transpose",
              [](TestContext& t) {
                  ...
                  const auto prod = mat({{1, 2}, {3, 4}}).multiply(mat({{5, 6}, {7, 8}})).value();
                  t.expect(prod == mat({{19, 22}, {43, 50}}), "2x2 product matches hand result");

                  // transpose of a 2x3
                  const auto tr = mat({{1, 2, 3}, {4, 5, 6}}).transpose().value();
        ...
        .test("solve_linear_systems",
              [](TestContext& t) {
                  ...
                  auto a2 = mat({{2, 1}, {1, -1}});
                  auto b2 = mat({{3}, {0}});
                  const auto x2 = a2.solve(b2).value();
                  ...
                  auto b3 = mat({{8}, {-11}, {-3}});
                  const auto x3 = a3.solve(b3).value();
        ...
        .test("overflow_is_reported",
              [](TestContext& t) {
                  ...
                  const auto a = Matrix::from_rows({{ri(big), ri(big)}, {ri(big), ri(big)}}).value();
