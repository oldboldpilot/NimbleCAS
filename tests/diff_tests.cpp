...
auto main() -> int {
    ...
    return TestSuite("nimblecas.diff")
        ...
        .test("sum_rule",
              [&](TestContext& t) {
                  // d/dx (x^2 + 3x) = 2x + 3
                  const auto u = x.pow(two).add(three.mul(x));
                  diffs_to(t, u, "x", two.mul(x).add(three), "d/dx (x^2 + 3x) = 2x + 3");
              })
        .test("product_rule",
              [&](TestContext& t) {
                  // d/dx (x * sin(x)) = sin(x) + x*cos(x)
                  const auto u = x.mul(fn("sin", x));
                  const auto expected = fn("sin", x).add(x.mul(fn("cos", x)));
        ...
        .test("chain_rule",
              [&](TestContext& t) {
                  ...
                  const auto expected = Expr::product({two, x, fn("cos", x.pow(two))});
        ...
        .test("special_functions",
              [&](TestContext& t) {
                  ...
                  const auto chained = Expr::product(
        ...
        .test("unknown_function_is_left_unevaluated",
              [&](TestContext& t) {
                  ...
                  const auto expected = Expr::apply("Derivative", {u, Expr::symbol("x")});
