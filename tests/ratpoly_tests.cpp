    ...
    return TestSuite("nimblecas.ratpoly")
        ...
        .test("division_with_remainder",
              [](TestContext& t) {
                  ...
                  const auto dm = ipoly({-1, 0, 1}).divide(ipoly({-1, 1})).value();
                  ...
                  const auto dm2 = ipoly({1, 0, 1}).divide(ipoly({0, 2})).value();
                  ...
                  const auto d = a.divide(b).value();
                  const auto qb = d.quotient.multiply(b).value();
                  const auto recon = qb.add(d.remainder).value();
