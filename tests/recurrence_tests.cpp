    ...
    return TestSuite("nimblecas.recurrence")
        .test("distinct_rational_roots",
              [](TestContext& t) {
                  ...
                  const auto poly = nimblecas::characteristic_polynomial(s).value();
                  t.expect(poly.is_equal(ipoly({6, -5, 1})), "char poly is x^2 - 5x + 6");

                  const auto roots = nimblecas::characteristic_roots(s).value();
        ...
        .test("closed_form_distinct_roots",
              [](TestContext& t) {
                  ...
                  const auto cf = nimblecas::closed_form(c, init).value();
        ...
        .test("generating_function_fibonacci",
              [](TestContext& t) {
                  ...
                  const auto c = coeffs({1, 1});
                  const auto init = coeffs({0, 1});
                  const auto gf = nimblecas::generating_function(c, init).value();
