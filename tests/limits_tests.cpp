...
auto main() -> int {
    ...
    return TestSuite("nimblecas.limits")
        .test("continuity_direct_substitution",
              [&](TestContext& t) {
                  // lim_{x->2} (x^2 + 1) = 5
                  const auto f = Expr::sum({powi(x, 2), intg(1)});
        ...
        .test("rational_at_infinity_equal_and_lower_degree",
              [&](TestContext& t) {
                  ...
                  const auto num = Expr::sum({Expr::product({intg(2), powi(x, 2)}), intg(3)});
                  const auto den = Expr::sum({powi(x, 2), intg(-1)});
        ...
        .test("honest_failures",
              [&](TestContext& t) {
                  ...
                  const auto osc = Expr::apply("sin", {powi(x, -1)});
                  ...
                  const auto transc = Expr::apply("exp", {x});
        ...
        .test("fractional_power_at_domain_boundary_is_not_guessed",
              [&](TestContext& t) {
                  ...
                  const auto sqrt_x = Expr::power(x, Expr::rational(1, 2).value());
        ...
        .test("crafted_polynomial_power_is_bounded_not_hung",
              [&](TestContext& t) {
                  ...
                  const auto inner = Expr::sum({powi(x, 256), intg(1)});
