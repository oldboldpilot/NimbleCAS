    ...
    return TestSuite("nimblecas.complex")
        ...
        .test("multiplication",
              [](TestContext& t) {
                  ...
                  const auto p = cx(1, 1).multiply(cx(1, -1)).value();
        ...
        .test("reciprocal",
              [](TestContext& t) {
                  // 1/i = -i
                  const auto r = Complex::i().reciprocal().value();
        ...
        .test("fractional_arithmetic",
              [](TestContext& t) {
                  ...
                  // z + conj(z) = 2*Re(z) = 1
                  const auto sum = z.add(z.conjugate().value()).value();
                  t.expect(sum == Complex::from_int(1), "z + conj(z) = 1");
                  // z * conj(z) = |z|^2 = 1/4 + 1/4 = 1/2
                  const auto prod = z.multiply(z.conjugate().value()).value();
