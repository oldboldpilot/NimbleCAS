    ...
    return TestSuite("nimblecas.nlsolve")
        ...
        .test("levenberg_marquardt_exponential_fit",
              [&](TestContext& t) {
                  ...
                  const auto resid = [ts, ys](std::span<const double> p) -> std::vector<double> {
        ...
        .test("domain_errors",
              [&](TestContext& t) {
                  ...
                  const auto wrong_dim = [](std::span<const double> x) -> std::vector<double> {
        ...
        .test("parallel_multistart_deterministic_best",
              [&](TestContext& t) {
                  ...
                  const nl::Solver solver = newton_solver;
                  const nl::Options o;
