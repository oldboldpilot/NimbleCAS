    ...
    return TestSuite("nimblecas.vectorcalc")
        ...
        .test("divergence_and_laplacian",
              [&](TestContext& t) {
                  ...
                  const std::vector<Expr> field = {Expr::power(x, I(2)), Expr::power(y, I(2)),
                                             Expr::power(z, I(2))};
                  const auto d = divergence(field, {"x", "y", "z"}).value();
        ...
        .test("curl",
              [&](TestContext& t) {
                  // curl(-y, x, 0) = (0, 0, 2).
                  const std::vector<Expr> field = {Expr::product({I(-1), y}), x, I(0)};
        ...
        .test("curl_of_gradient_is_zero",
              [&](TestContext& t) {
                  ...
                  const auto g = gradient(f, {"x", "y", "z"}).value();
        ...
        .test("div_of_curl_is_zero",
              [&](TestContext& t) {
                  ...
                  const std::vector<Expr> field = {Expr::product({Expr::power(x, I(2)), y}),
                                             Expr::product({Expr::power(y, I(2)), z}),
                                             Expr::product({Expr::power(z, I(2)), x})};
                  const auto c = curl(field, {"x", "y", "z"}).value();
        ...
        .test("jacobian_and_hessian",
              [&](TestContext& t) {
                  ...
                  const std::vector<Expr> field = {Expr::product({x, y}), Expr::product({y, z})};
        ...
        .test("directional_and_total_derivative",
              [&](TestContext& t) {
                  ...
                  const Expr f = Expr::sum({Expr::power(x, I(2)), Expr::power(y, I(2))});
                  const auto dd = directional_derivative(f, {"x", "y"}, {I(1), I(0)}).value();
                  ...
                  const auto td = total_derivative(Expr::product({x, y}), "t", {"x", "y"}, {u, v})
                                .value();
