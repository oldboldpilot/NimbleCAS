...
auto main() -> int {
    ...
    return TestSuite("nimblecas.simplify")
        ...
        .test("canonical_ordering",
              [&](TestContext& t) {
                  const auto lhs = simplify(x.add(y)).value();
                  const auto rhs = simplify(y.add(x)).value();
        ...
        .test("combine_like_terms",
              [&](TestContext& t) {
                  ...
                  // 2*x + 3*x -> 5*x
                  const auto lhs = Expr::integer(2).mul(x).add(Expr::integer(3).mul(x));
        ...
        .test("combine_like_bases",
              [&](TestContext& t) {
                  ...
                  // x^2 * x^3 -> x^5
                  const auto lhs = x.pow(Expr::integer(2)).mul(x.pow(Expr::integer(3)));
        ...
        .test("reconstructed_terms_stay_flat",
              [&](TestContext& t) {
                  ...
                  const auto lhs = Expr::product({Expr::integer(2), x, y})
        ...
        .test("no_grouping_collision_via_stringification",
              [&](TestContext& t) {
                  ...
                  const auto sym = Expr::symbol("f(x)");
                  const auto call = Expr::apply("f", {x});
