    ...
    return TestSuite("nimblecas.tensor")
        .test("flat_metric_is_curvature_free",
              [&](TestContext& t) {
                  ...
                  const auto riem = riemann_tensor(m).value();
                  bool riem_zero = true;
                  ...
                  const auto ric = ricci_tensor(m).value();
        ...
        .test("sphere_christoffel_symbols",
              [&](TestContext& t) {
                  ...
                  const auto m = make_metric({"theta", "phi"}, g).value();
        ...
        .test("sphere_curvature_two_over_a_squared",
              [&](TestContext& t) {
                  ...
                  const auto ein = einstein_tensor(m).value();
        ...
        .test("polar_laplace_beltrami_reproduces_polar_laplacian",
              [&](TestContext& t) {
                  ...
                  const auto m = make_metric({"r", "theta"}, g).value();
                  ...
                  const auto d = [](const Expr& e, std::string_view v) {
        ...
        .test("error_and_boundary_cases",
              [&](TestContext& t) {
                  ...
                  const auto singular = make_metric({"u", "v"}, {{I(1), I(1)}, {I(1), I(1)}}).value();
                  ...
                  const auto big = make_metric({"a", "b", "c", "d", "e", "f"},
                  ...
                  const auto flat2 = make_metric({"x", "y"}, identity_components(2)).value();
