    ...
    return TestSuite("nimblecas.splines")
        .test("natural_spline_through_collinear_is_the_line",
              [](TestContext& t) {
                  ...
                  const auto sp = CubicSpline::natural(xs, ys).value();
                  ...
                  for (const auto& x : probes) {
                      const auto expected = x.multiply(ri(3)).value().subtract(ri(1)).value();  // 3x - 1
        ...
        .test("clamped_spline_reproduces_a_cubic_and_is_C2",
              [](TestContext& t) {
                  ...
                  const auto sp = CubicSpline::clamped(xs, ys, peval(qp, xs.front()),
                                                 peval(qp, xs.back()))
                                .value();
        ...
        .test("periodic_spline_is_C2_and_wraps",
              [](TestContext& t) {
                  ...
                  const auto sp = CubicSpline::periodic(xs, ys).value();
        ...
        .test("hermite_from_slopes_and_pchip",
              [](TestContext& t) {
                  ...
                  const auto hs = HermiteSpline::from_slopes(xs, ys, slopes).value();
                  for (std::size_t i = 0; i < xs.size(); ++i) {
                  ...
                  const auto pc = HermiteSpline::pchip(px, py).value();
        ...
        .test("bezier_de_casteljau_bernstein_and_subdivision",
              [](TestContext& t) {
                  const auto bez = BezierCurve::make(rats({1, 3, 2, 5})).value();
                  ...
                  const auto split = nimblecas::bezier_subdivide(bez, half).value();
        ...
        .test("bezier_degree_one_is_lerp",
              [](TestContext& t) {
                  const auto bez = BezierCurve::make(rats({2, 8})).value();
        ...
        .test("bezier_power_basis_roundtrip",
              [](TestContext& t) {
                  const auto bez = BezierCurve::make(rats({1, 3, 2, 5})).value();
                  const auto power = bez.to_power_basis().value();
                  const auto back = BezierCurve::from_power_basis(power, bez.degree()).value();
        ...
        .test("bezier2_two_dimensional",
              [](TestContext& t) {
                  const std::vector<Point2> cps{Point2{.x = ri(0), .y = ri(0)},
                                          Point2{.x = ri(2), .y = ri(4)}};
                  const auto bez = nimblecas::BezierCurve2::make(cps).value();
                  const auto p = bez.evaluate(rat(1, 2)).value();
                  t.expect(p == (Point2{.x = ri(1), .y = ri(2)}), "2-D Bezier midpoint exact");
                  const auto elevated = bez.elevate().value();
        ...
        .test("bspline_partition_of_unity_and_endpoint_interpolation",
              [](TestContext& t) {
                  ...
                  const auto knots = rats({0, 0, 0, 1, 2, 3, 3, 3});
                  auto ctrl = rats({1, 2, 0, 3, 1});
                  const auto bs = BSpline::make(knots, ctrl, 2).value();
        ...
        .test("bspline_open_knot_vector_endpoint_is_not_clamped",
              [](TestContext& t) {
                  ...
                  const auto knots = rats({0, 1, 2, 3, 4});
                  const auto ctrl = rats({1, 2, 3});
                  const auto bs = BSpline::make(knots, ctrl, 1).value();
        ...
        .test("nurbs_rational_weights_exact",
              [](TestContext& t) {
                  ...
                  const auto knots = rats({0, 0, 1, 1});
                  const auto ctrl = rats({0, 2});
                  const auto weights = rats({1, 3});
                  const auto nb = NurbsCurve::make(knots, ctrl, weights, 1).value();
        ...
        .test("domain_errors",
              [](TestContext& t) {
                  ...
                  const auto bez = BezierCurve::make(rats({1, 2, 3})).value();
                  ...
                  const auto knots = rats({0, 0, 1, 1});
                  const auto ctrl = rats({0, 1, 2});  // wrong count for degree 1 (needs 2)
                  ...
                  const auto nk = rats({0, 0, 1, 1});
                  const auto nc = rats({0, 2});
                  const auto nw = rats({1});  // wrong weight count
