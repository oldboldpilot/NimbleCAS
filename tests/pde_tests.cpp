...
[[nodiscard]] auto wave_residual_holds(const nimblecas::SpatialOperator& l,
    ...
    for (std::size_t n = 0; n + 2 < c.size(); ++n) {
        const auto factor = Rational::from_int(static_cast<std::int64_t>((n + 2) * (n + 1)));
    ...
    return TestSuite("nimblecas.pde")
        .test("heat_x2_diffusivity_one",
              [](TestContext& t) {
                  ...
                  const auto l = nimblecas::heat_operator(Rational::from_int(1));
        ...
        .test("transport_x2_speed_one",
              [](TestContext& t) {
                  ...
                  const auto l = nimblecas::transport_operator(Rational::from_int(1));
                  auto c = nimblecas::solve_evolution_pde(l, ipoly({0, 0, 1}), 3).value();
                  t.expect(c[0].is_equal(ipoly({0, 0, 1})), "c_0 = x^2");
        ...
        .test("heat_x4_diffusivity_half",
              [](TestContext& t) {
                  ...
                  const auto l = nimblecas::heat_operator(rat(1, 2));
                  auto c = nimblecas::solve_evolution_pde(l, ipoly({0, 0, 0, 0, 1}), 3).value();
                  t.expect(c[0].is_equal(ipoly({0, 0, 0, 0, 1})), "c_0 = x^4");
        ...
        .test("transport_x2_speed_half",
              [](TestContext& t) {
                  ...
                  const auto l = nimblecas::transport_operator(rat(1, 2));
                  auto c = nimblecas::solve_evolution_pde(l, ipoly({0, 0, 1}), 3).value();
                  t.expect(c[0].is_equal(ipoly({0, 0, 1})), "c_0 = x^2");
        ...
        .test("transport_x3_speed_three",
              [](TestContext& t) {
                  ...
                  const auto l = nimblecas::transport_operator(Rational::from_int(3));
                  auto c = nimblecas::solve_evolution_pde(l, ipoly({0, 0, 1}), 3).value();
                  t.expect(c[0].is_equal(ipoly({0, 0, 1})), "c_0 = x^2");
        ...
        .test("advection_diffusion_combined",
              [](TestContext& t) {
                  ...
                  // L[1] = 0 => c_3 = 0.
                  const auto l = nimblecas::advection_diffusion(Rational::from_int(1),
                                                          Rational::from_int(1));
                  auto c = nimblecas::solve_evolution_pde(l, ipoly({0, 0, 1}), 3).value();
                  t.expect(c[0].is_equal(ipoly({0, 0, 1})), "c_0 = x^2");
        ...
        .test("evaluate_horner_rational_point",
              [](TestContext& t) {
                  ...
                  const auto l = nimblecas::heat_operator(Rational::from_int(1));
                  const auto c = nimblecas::solve_evolution_pde(l, ipoly({0, 0, 0, 0, 1}), 2).value();
        ...
        .test("error_paths",
              [](TestContext& t) {
                  ...
                  const nimblecas::SpatialOperator failing =
        ...
        .test("nonlinear_error_paths",
              [](TestContext& t) {
                  ...
                  const nimblecas::TimeSeriesOperator failing =
        ...
        .test("poisson_uxx_2_zero_dirichlet",
              [](TestContext& t) {
                  ...
                  const auto u = nimblecas::solve_poisson_bvp_1d(ipoly({2}), Rational::from_int(0),
                                                           Rational::from_int(0),
                                                           Rational::from_int(1),
                                                           Rational::from_int(0))
                               .value();
        ...
        .test("poisson_uxx_x_zero_dirichlet",
              [](TestContext& t) {
                  ...
                  const auto u = nimblecas::solve_poisson_bvp_1d(ipoly({0, 1}), Rational::from_int(0),
                                                           Rational::from_int(0),
                                                           Rational::from_int(1),
                                                           Rational::from_int(0))
                               .value();
        ...
        .test("laplace_linear_interpolation",
              [](TestContext& t) {
                  ...
                  const auto u = nimblecas::solve_poisson_bvp_1d(RationalPoly{}, Rational::from_int(0),
                                                           Rational::from_int(1),
                                                           Rational::from_int(2),
                                                           Rational::from_int(5))
                               .value();
        ...
        .test("wave_x2_speed_one_static",
              [](TestContext& t) {
                  ...
                  const auto l = nimblecas::heat_operator(Rational::from_int(1));  // L = 1 * d^2/dx^2
