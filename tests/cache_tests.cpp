    ...
    return TestSuite("nimblecas.cache")
        .test("computes_once_for_structurally_equal_keys",
              [](TestContext& t) {
                  ...
                  const auto compute = [&] -> Result<Expr> {
                      calls.fetch_add(1, std::memory_order_relaxed);
                      return Expr::symbol("result");
                  };
                  // two distinct objects, structurally equal
                  const auto k1 = Expr::symbol("x").add(Expr::integer(1));
                  const auto k2 = Expr::symbol("x").add(Expr::integer(1));
        ...
        .test("error_results_are_cached",
              [](TestContext& t) {
                  ...
                  const auto r2 = memo.get_or_compute(key, constant_fn(Expr::integer(0)));
