    ...
    return TestSuite("nimblecas.stats")
        ...
        .test("covariance_matrix_correlated",
              [](TestContext& t) {
                  ...
                  const auto sigma = nimblecas::covariance_matrix(vars, true).value();
                  const auto one = Rational::from_int(1);
                  const auto expected = Matrix::from_rows({{one, one}, {one, one}}).value();
        ...
        .test("covariance_matrix_anticorrelated",
              [](TestContext& t) {
                  ...
                  const auto expected = Matrix::from_rows({{one, neg}, {neg, one}}).value();
        ...
        .test("error_cases",
              [](TestContext& t) {
                  ...
                  const std::vector<std::span<const Rational>> ragged{std::span<const Rational>{a},
                  ...
                  const std::vector<std::span<const Rational>> no_vars;
        ...
        .test("excess_kurtosis_and_skewness_squared",
              [](TestContext& t) {
                  ...
                  const auto m3 = nimblecas::central_moment(std::span<const Rational>{skew}, 3).value();
        ...
        .test("pearson_correlation_squared",
              [](TestContext& t) {
                  ...
                  const std::vector<std::span<const Rational>> vars{std::span<const Rational>{x},
                                                              std::span<const Rational>{yp}};
                  const auto r2 = nimblecas::correlation_squared_matrix(vars).value();
                  const auto one = Rational::from_int(1);
