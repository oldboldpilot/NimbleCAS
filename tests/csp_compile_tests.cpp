...
auto main() -> int {
    return TestSuite("nimblecas.csp_compile")
        .test("the_exhaustive_reference_returns_what_backtracking_search_returns",
              [](TestContext& t) {
                  ...
                  for (const std::int64_t n : {4, 5, 6}) {
                      ...
                      const auto want = backtracking_search(*csp);
                      const auto got = reference_exhaustive(w);
        ...
        .test("min_conflicts_is_reproducible_from_its_seed",
              [](TestContext& t) {
                  const auto w = wire_queens_pairwise(8);
                  const auto a = reference_min_conflicts(w, 4, 20000, 200, 0xABCDULL);
                  const auto b = reference_min_conflicts(w, 4, 20000, 200, 0xABCDULL);
        ...
        .test("emission_is_deterministic",
              [](TestContext& t) {
                  ...
                  for (const auto target : {Target::cpp, Target::cuda, Target::triton}) {
                      EmitOptions opts;
                      opts.target = target;
                      const auto a = emit(w, opts);
                      const auto b = emit(w, opts);
