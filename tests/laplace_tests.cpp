    ...
    return TestSuite("nimblecas.laplace")
        ...
        .test("power_of_t",
              [&](TestContext& t_ctx) {
                  ...
                  // L{t^2} = 2/s^3
                  const auto t2 = Expr::power(t, Expr::integer(2));
        ...
        .test("exponential",
              [&](TestContext& t_ctx) {
                  // L{exp(2 t)} = 1/(s - 2)
                  const auto f = Expr::apply("exp", {Expr::product({Expr::integer(2), t})});
        ...
        .test("sine_and_cosine",
              [&](TestContext& t_ctx) {
                  // L{sin(3 t)} = 3/(s^2 + 9)
                  const auto fsin = Expr::apply("sin", {Expr::product({Expr::integer(3), t})});
                  ...
                  // L{cos(t)} = s/(s^2 + 1)
                  const auto fcos = Expr::apply("cos", {t});
        ...
        .test("linearity",
              [&](TestContext& t_ctx) {
                  // L{2 + 3 t} = 2/s + 3/s^2
                  const auto f = Expr::sum({Expr::integer(2), Expr::product({Expr::integer(3), t})});
                  auto result = laplace_transform(f, "t", "s");
        ...
        .test("unhandled_is_not_implemented",
              [&](TestContext& t_ctx) {
                  ...
                  const auto flog = Expr::apply("log", {t});
                  ...
                  const auto prod = Expr::product({t, Expr::apply("sin", {t})});
        ...
        .test("inverse_exponential",
              [&](TestContext& t_ctx) {
                  // L^{-1}{1/(s-2)} = e^(2 t)
                  const auto F = Expr::power(Expr::sum({s, Expr::integer(-2)}), Expr::integer(-1));
        ...
        .test("inverse_power_and_repeated_pole",
              [&](TestContext& t_ctx) {
                  // L^{-1}{1/s^2} = t
                  const auto Ft = Expr::power(s, Expr::integer(-2));
                  ...
                  // L^{-1}{1/(s-1)^2} = t e^t
                  const auto Fte = Expr::power(Expr::sum({s, Expr::integer(-1)}), Expr::integer(-2));
