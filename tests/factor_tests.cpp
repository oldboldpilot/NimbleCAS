...
auto main() -> int {
    return TestSuite("nimblecas.factor")
        .test("difference_of_squares_two_linear",
              [](TestContext& t) {
                  // x^2 - 1 = (x - 1)(x + 1)
                  const auto f = factor_over_Q(ipoly({-1, 0, 1})).value();
        ...
        .test("irreducible_quadratic",
              [](TestContext& t) {
                  ...
                  const auto f = factor_over_Q(ipoly({-2, 0, 1})).value();
        ...
        .test("reducible_quintic_no_rational_roots",
              [](TestContext& t) {
                  ...
                  const auto f = factor_over_Q(ipoly({4, 0, -2, -2, 0, 1})).value();
                  t.expect(f.size() == 2, "two irreducible factors");
        ...
        .test("repeated_linear_multiplicity",
              [](TestContext& t) {
                  // (x - 1)^2 (x + 2) = x^3 - 3x + 2
                  const auto f = factor_over_Q(ipoly({2, -3, 0, 1})).value();
        ...
        .test("irreducible_quartic",
              [](TestContext& t) {
                  ...
                  const auto f = factor_over_Q(ipoly({1, 0, 0, 0, 1})).value();
                  t.expect(f.size() == 1, "a single irreducible factor");
        ...
        .test("reducible_sextic_three_quadratics",
              [](TestContext& t) {
                  ...
                  const auto f = factor_over_Q(ipoly({-2, -2, -3, -1, 0, 1, 1})).value();
                  t.expect(f.size() == 3, "three irreducible factors");
        ...
        .test("degenerate_inputs",
              [](TestContext& t) {
                  ...
                  const auto f = factor_over_Q(ipoly({5})).value();
