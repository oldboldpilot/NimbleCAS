    ...
    return TestSuite("nimblecas.yieldcurve")
        ...
        .test("zero2fwd . fwd2zero identity, and flat forward == zero",
              [](TestContext& t) {
                  ...
                  const auto ff = zero2fwd(times, flat, Compounding::continuous).value();
                  bool allflat = true;
                  for (const double f : ff) { allflat = allflat && close(f, 0.05, 1e-10); }
        ...
        .test("bootstrap zbtprice reproduces input bond prices",
              [](TestContext& t) {
                  ...
                  const auto reprice = [&](const CouponBond& b) -> double {
                  ...
                  const auto df = cv.pillar_times();
                  bool mono = true;
                  double prev = 1.0;
                  for (const double tt : df) {
        ...
        .test("interpolation modes and Hull-White lattice reprices the curve",
              [](TestContext& t) {
                  ...
                  const auto lz = Curve::create({1.0, 2.0}, {0.03, 0.05}, Interp::linear_zero,
                                          Compounding::continuous)
                                .value();
                  ...
                  const auto ld = Curve::create({1.0, 2.0}, {0.03, 0.05}, Interp::loglinear_df,
                                          Compounding::continuous)
                                .value();
        ...
        .test("calendar bridge reuses finance Date/DayCount",
              [](TestContext& t) {
                  ...
                  const auto curve = Curve::create(times.value(), {0.02, 0.025, 0.03},
