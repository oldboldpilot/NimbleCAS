...
auto main() -> int {
    return TestSuite("nimblecas.symbolic")
        ...
        .test("hash_matches_equality",
              [](TestContext& t) {
                  const auto a = Expr::symbol("x").add(Expr::integer(1));
                  const auto b = Expr::symbol("x").add(Expr::integer(1));
        ...
        .test("nan_leaf_is_equivalent_to_itself",
              [](TestContext& t) {
                  const auto nan_expr = Expr::real(std::numeric_limits<double>::quiet_NaN());
        ...
        .test("free_of",
              [](TestContext& t) {
                  ...
                  const auto y = Expr::symbol("y");
                  // u = x^2 + y
                  const auto u = Expr::power(x, Expr::integer(2)).add(y);
        ...
        .test("substitute_replaces_subexpression",
              [](TestContext& t) {
                  ...
                  const auto u = Expr::power(x, Expr::integer(2)).add(x);
                  const auto expected = Expr::power(y, Expr::integer(2)).add(y);
                  const auto result = substitute(u, x, y);
        ...
        .test("substitute_whole_expression",
              [](TestContext& t) {
                  const auto x = Expr::symbol("x");
                  const auto expr = Expr::power(x, Expr::integer(2));
                  const auto r = substitute(expr, expr, Expr::integer(7));
