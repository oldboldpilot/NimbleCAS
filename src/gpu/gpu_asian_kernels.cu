// NimbleCAS CUDA GPU kernels — batched Asian Monte Carlo pricing.
...
// Internal POD for precalculated option parameters in Asian MC simulation.
struct AsianOptPre {
    double spot;
    double strike;
    double drift;       // (rate - div - 0.5 * vol * vol) * dt
    double vol_sqrtdt;  // vol * sqrt(dt)
...
__global__ void asian_segment_kernel(const AsianOptPre* __restrict__ pre, int n_opts,
                                     int steps, unsigned long long paths, int nseg,
    ...
    for (long long idx = static_cast<long long>(blockIdx.x) * blockDim.x + threadIdx.x;
        ...
        for (unsigned long long p = start_p; p < end_p; ++p) {
            ...
            const unsigned long long path_base = p * static_cast<unsigned long long>(steps);
            for (int t = 0; t < steps; ++t) {
                const unsigned long long ctr_idx = path_base + static_cast<unsigned long long>(t);
            ...
            const double payoff = (o.is_call ? fmax(arith_avg - o.strike, 0.0) : fmax(o.strike - arith_avg, 0.0)) * o.disc;
            sum += payoff;
...
__global__ void asian_reduce_kernel(const double* __restrict__ psum,
...
extern "C" int nimblecas_gpu_mc_asian_batch(const NimblecasBsOption* opts, int n, int steps,
    ...
    const size_t pre_bytes = static_cast<size_t>(n) * sizeof(AsianOptPre);
    ...
    AsianOptPre* pre_host = (AsianOptPre*)malloc(pre_bytes);
    ...
    if ((err = cudaMalloc(&dev_pre, pre_bytes)) != cudaSuccess) {
        ...
        asian_segment_kernel<<<blocks, threads>>>(dev_pre, n, steps, paths, nseg, key, dev_psum, dev_psumsq);
        if ((err = cudaGetLastError()) != cudaSuccess) {
            rc = static_cast<int>(err);
        } else if ((err = cudaDeviceSynchronize()) != cudaSuccess) {
            rc = static_cast<int>(err);
        } else {
            asian_reduce_kernel<<<n, 256, 2 * 256 * sizeof(double)>>>(dev_psum, dev_psumsq, nseg, dev_sums, dev_sumsqs);
            if ((err = cudaGetLastError()) != cudaSuccess) {
                rc = static_cast<int>(err);
            } else if ((err = cudaDeviceSynchronize()) != cudaSuccess) {
                rc = static_cast<int>(err);
