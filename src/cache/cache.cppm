...
public:
    ...
    // Variant for total (non-failing) transforms such as raw differentiation.
    ...
    [[nodiscard]] auto get_or_compute_value(const Expr& key,
...
auto ExprMemo::get_or_compute(const Expr& key, const std::function<Result<Expr>()>& compute)
    ...
    // 1. Fast path: look up under the shard lock.
    {
        ...
        if (const auto it = shard.buckets.find(h); it != shard.buckets.end()) {
