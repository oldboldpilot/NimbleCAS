...
void hessenberg_orthes(std::vector<double>& H, std::vector<double>& V, std::ptrdiff_t N) {
    ...
    const auto Hx = [&](std::ptrdiff_t i, std::ptrdiff_t j) -> double& {
        return H[static_cast<std::size_t>(i * N + j)];
    };
    const auto Vx = [&](std::ptrdiff_t i, std::ptrdiff_t j) -> double& {
        return V[static_cast<std::size_t>(i * N + j)];
    };
    const auto O = [&](std::ptrdiff_t i) -> double& { return ort[static_cast<std::size_t>(i)]; };
