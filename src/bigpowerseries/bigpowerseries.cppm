...
auto BigPowerSeries::derivative() const -> Result<BigPowerSeries> {
    ...
    for (std::size_t k = 0; k + 1 < n; ++k) {
        const auto factor = BigRational::from_int(static_cast<std::int64_t>(k + 1));
...
auto BigPowerSeries::integrate() const -> Result<BigPowerSeries> {
    ...
    for (std::size_t k = 1; k < n; ++k) {
        const auto denom = BigRational::from_int(static_cast<std::int64_t>(k));
...
auto BigPowerSeries::exp() const -> Result<BigPowerSeries> {
    ...
    for (std::size_t k = 1; k < n; ++k) {
        ...
        BigRational s;  // 0
        for (std::size_t i = 1; i <= k; ++i) {
            const auto scaled = coeffs_[i].multiply(BigRational::from_int(static_cast<std::int64_t>(i)));
