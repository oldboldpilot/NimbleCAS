...
auto BigAlgebraicNumber::multiply(const BigAlgebraicNumber& o) const
    ...
    const BigRationalPoly prod = value_.multiply(o.value_);   // degree up to 2d-2 (infallible)
...
auto BigAlgebraicNumber::mult_matrix() const
    ...
    for (std::size_t j = 0; j < dd; ++j) {
        ...
        if (j + 1 < dd) {
            const BigRationalPoly prod = current.multiply(x);  // multiply by alpha (infallible)
