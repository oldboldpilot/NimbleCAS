...
| `monte_carlo_european_parallel` | `auto monte_carlo_european_parallel(const OptionSpec& spec, std::uint64_t paths, std::uint64_t seed) -> Result<McResult>` | The parallel counterpart: identical model and draw stream, but paths are priced in a **fixed block decomposition** across the fork-join runtime ([`parallel`](parallel.md)) and the block partials combined in index order. **Deterministic** — the result depends only on `(spec, paths, seed)`, never on thread count or scheduling (asserted bit-identical across calls). It equals `monte_carlo_european` to **FP tolerance** (`\|Δprice\| ≈ 1.7e-12`, i.e. non-associativity of the sum, far below the MC standard error), *not* bit-for-bit. A `perf` pass (Xeon Gold 6152) showed European MC is **compute/transcendental-bound** (IPC 2.09, ~25 % in scalar libm `log`+`exp`, RNG core already AVX-512), so it scales near-linearly with cores: **`cpu`/`mc_bench` measures ~29× (28.7 → 840 M paths/s)** on that 88-thread box. Same guards as the serial pricer. |
