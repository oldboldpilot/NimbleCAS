...
| `nimblecas.taskdag_sgee` | [taskdag_sgee.md](reference/taskdag_sgee.md) | SGEE-backed distributed `Executor` backend (`SgeeDistributedExecutor`): coordinator-side level scheduling over SGEE's task broker queue, out-of-band result channel, binary codec ("NCDT"/"NCRT"), worker pump, and deterministic `FakeBrokerPort` (bit-identical outputs to `serial_executor`). |
