...
| `nimblecas.memo_dist` | [memo_dist.md](reference/memo_dist.md) | Content-addressed memoization for distributed execution ("distributed hash-consing"): a process-stable 128-bit `ContentKey` over canonical task bytes (deliberately NOT `std::hash`), the `DistributedMemo` interface, and a sharded bounded `InProcessMemo`. Every hit is confirmed by a full-key byte comparison, so a fingerprint collision costs a lost entry and never a wrong answer. |
