...
| `nimblecas.taskdag` | [taskdag.md](reference/taskdag.md) | Deterministic single-node task-DAG scheduler over `parallel`: an acyclic-by-construction `TaskGraph`, wavefront `serial_executor`/`local_parallel_executor` (bit-identical outputs), `TaskRegistry` and `add_named_task` for named ops, and deterministic error poisoning — a documented backend seam. |
| `nimblecas.taskdag_sched` | [taskdag_sched.md](reference/taskdag_sched.md) | Cost-aware, deterministic wavefront task scheduling over `taskdag`: three-tier cost resolution (`CostHint` → frozen `CostTable` → unknown), sanitized LPT ordering with variance risk margin (tie-break on ascending `TaskId`), and `cost_ordered_local_executor` (outputs bit-identical to `serial_executor`). |
| `nimblecas.taskdag_sgee` | [taskdag_sgee.md](reference/taskdag_sgee.md) | SGEE-backed distributed `Executor` backend (`SgeeDistributedExecutor`): coordinator-side level scheduling over SGEE's task broker queue, out-of-band result channel, binary codec ("NCDT"/"NCRT"), worker pump, and deterministic `FakeBrokerPort` (bit-identical outputs to `serial_executor`). Cross-process over gRPC, and against a 3-node Raft quorum with opt-in mTLS and leader failover. |
...
| Triton CSR SpMV | [triton-csr-spmv.md](reference/triton-csr-spmv.md) | GPU/Triton mirror of `krylov::csr_matvec` (`python/triton/csr_spmv.py`), an **ncu-tuned** rows-per-program kernel: matches `torch.sparse` to machine precision on the RTX 5090 and, after an `ncu`-guided occupancy fix (50→100 % occupancy, 35→85 % DRAM), runs at **~1162 GB/s fp64 (~68 % of peak, 2.7× the naïve kernel)**. Also records the reasoned decision **not** to ship a CPU SIMD `csr_matvec` (its fused-fma reduction can't be bit-identically vectorized by the elementwise-only `nimblecas.simd`). |
...
| `nimblecas.modgcd` | [modgcd.md](reference/modgcd.md) | Brown's modular GCD for univariate Z[x]: Landau–Mignotte bound, Z_p[x] Euclidean GCD kernel, symmetric CRT merge, and trial-division verification. |
| `nimblecas.memo_dist` | [memo_dist.md](reference/memo_dist.md) | Content-addressed memoization for distributed execution ("distributed hash-consing"): a process-stable 128-bit `ContentKey` over canonical task bytes (deliberately NOT `std::hash`), the `DistributedMemo` interface, a sharded bounded `InProcessMemo`, and a durable append-only `FileMemo` that survives the process. Every hit is confirmed by a full-key byte comparison, so a fingerprint collision costs a lost entry and never a wrong answer. |
| `nimblecas.memo_postgres` | [memo_postgres.md](reference/memo_postgres.md) | A `DistributedMemo` backed by a PostgreSQL table, so a memo is shared by executors with no filesystem in common. OPT-IN behind `NIMBLECAS_POSTGRES` -- the only module with a third-party runtime dependency (libpq). Every transport failure is `distributed_error` and NEVER a miss; a fingerprint collision returns a miss, never the colliding value. |
| `nimblecas.modgcd_dist` | [modgcd_dist.md](reference/modgcd_dist.md) | Distributed driver for Brown's modular GCD over `Executor`: per-prime image tasks with bound literals, coordinator-local CRT merge, trial-division verification. |
...
| `nimblecas.planning_dist` | [planning_dist.md](reference/planning_dist.md) | Distributed SAS+ planning over `taskdag`: the coordinator owns the open and closed lists, workers expand and evaluate and own nothing. A* expands a whole f-layer per round, which is a legal A* order and so keeps OPTIMALITY -- the plan costs exactly what `astar_plan`'s does, though where several optima exist it may be a different one. Satisficing greedy search with a configurable beam; every plan replayed before it is returned. |
...
| `nimblecas.daenl` | [daenl.md](reference/daenl.md) | **Nonlinear / higher-index** DAEs, three honesty tiers: `solve_semiexplicit_nonlinear_series` (exact power series over `Q` when `dg/dy` is a constant matrix and `f`/`g` use `+ * ^ exp ln`), `analyze_structure` (Pantelides-style structural index via `diff` + bipartite matching), and numerical index-1 implicit-Euler/BDF-2 stepping (`solve_nonlinear_dae`/`consistent_initial_values` via `nlsolve`). Out-of-grammar / higher-index / non-convergent cases are honest `not_implemented`/`not_converged`. |
...
| `nimblecas.marketdata` | [marketdata.md](reference/marketdata.md) | Provider-agnostic market-data ingestion: one normalised model (`Quote`/`Bar`/`BarSeries`/`OptionQuote`/`OptionChain`/`RateQuote`) plus a thin adapter per provider (**Yahoo Finance / Alpha Vantage / Alpaca**) mapping each provider's JSON into it via the vendored **fastjson** (fastestjsoninthewest, Rule 37) — adding a provider is one enum value + one adapter. Exact proleptic-Gregorian epoch calendar, a fluent/composable/reusable `Feed` façade, and a strict honesty contract (malformed JSON → `syntax_error`, missing/wrong-typed field → `domain_error`, unwired path → `not_implemented`; never a fabricated quote). |
...
| `nimblecas.fixedincome` | [fixedincome.md](reference/fixedincome.md) | Fixed-income sensitivities & odd-period pricing over [`finance`](reference/finance.md): DV01/PV01, duration-from-price, effective & key-rate durations; `cfdates`/`cfamounts` dated cashflows; COUPPCD/COUPNCD/COUPDAYBS/COUPDAYS/COUPDAYSNC (with `bs + nc == full`); ODDFPRICE/ODDFYIELD/ODDLPRICE/ODDLYIELD (matching `bond_price` bit-for-bit on 30/360); PRICEMAT/YIELDMAT/ACCRINTM; continuous Z-spread; simple↔compound / nominal converters; FRN valuation and level-payment amortization. |
| `nimblecas.yieldcurve` | [yieldcurve.md](reference/yieldcurve.md) | Term-structure toolkit reusing [`finance`](reference/finance.md) dates: pillar `Curve` (linear-on-zero / log-linear-on-DF interpolation, continuous/annual/semiannual/k-per-year compounding), `zero2disc`/`disc2zero`/`zero2fwd`/`fwd2zero`, par↔zero exact inverses, bootstrapping (`zbtprice`/`zbtyield`), Nelson-Siegel & Svensson fits, and a Hull-White trinomial short-rate lattice calibrated to the input curve by Arrow-Debreu induction — with callable-bond backward induction and an option-adjusted-spread (OAS) solver on that lattice. No-extrapolation → `domain_error`, non-convergent fit/OAS → `not_converged`. |
| `nimblecas.futures` | [futures.md](reference/futures.md) | Futures & forwards (numerical): the cost-of-carry model `F = S·e^{bT}` (carry `b = r − q + u − y`), discrete-income forwards, basis + contango/backwardation, implied cost-of-carry / convenience yield, off-market forward value, MtM P&L, notional/tick arithmetic; plus a **composable, fluent strategy layer** — signed `FuturesLeg` + reusable `FuturesStrategy` with named builders (outright, calendar spread, inter-commodity/crack-crush spread, long/short hedge, cash-and-carry) — futures==forward only under deterministic rates, outright P&L reported as truthfully unbounded. |
| `nimblecas.optstrat` | [optstrat.md](reference/optstrat.md) | Composable option trading strategies over [`pricing`](reference/pricing.md): signed `StrategyLeg`s (call/put/underlying) + `OptionStrategy` with an **exact piecewise-linear expiry-P&L analytics engine** (net premium debit/credit, max profit, max loss, all breakevens; unbounded directions reported truthfully). Named builders for the full zoo — covered call, protective put, collar, bull/bear call & put spreads, straddle, strangle, strip/strap, call & iron butterfly, call & iron condor, box, risk reversal, ratio — plus aggregate Black-Scholes Greeks via a pricing bridge. |
| `nimblecas.exotics` | [exotics.md](reference/exotics.md) | Exotic-option pricing over [`pricing`](reference/pricing.md): Cox-Ross-Rubinstein binomial (European/American + escrowed discrete dividends), Reiner-Rubinstein analytic single barriers (8 types + rebate), Goldman-Sosin-Gatto lookback, a Crank-Nicolson/Rannacher finite-difference PDE (Thomas/PSOR), and Margrabe/Kirk/Geske/chooser/basket via a Drezner bivariate-normal CDF — reusing the counter-based RNG and Black-Scholes core, never NaN/inf. |
| `nimblecas.riskextra` | [riskextra.md](reference/riskextra.md) | Extended risk & annuity toolkit over [`rng`](reference/rng.md): `corr2cov`/`cov2corr`, lower partial moments, exponentially-weighted moments, Cholesky-correlated simulation, a box-constrained mean-variance frontier (active-set QP), Rockafellar-Uryasev CVaR-optimal weights over a self-contained two-phase simplex, AMORLINC/AMORDEGRC depreciation, and continuous-compounding EFFECT/NOMINAL + `amortize`/`pay_per`/`pay_odd`/`pay_uni`. |
| `nimblecas.fxstrat` | [fxstrat.md](reference/fxstrat.md) | Currency strategies (numerical): Garman-Kohlhagen FX options (reusing [`pricing`](reference/pricing.md)), covered-interest-parity forwards & forward points, the covered-interest-arbitrage mispricing, and a fluent `CarryTrade` whose breakeven exit spot is exactly the CIP forward (uncovered interest parity). |
| `nimblecas.bondstrat` | [bondstrat.md](reference/bondstrat.md) | Bond portfolio construction & relative value (exact weighting algebra over [`finance`](reference/finance.md) durations): weighted portfolio duration/convexity/yield, barbell weights to a target duration, duration- & cash-neutral butterfly, duration-neutral hedge ratio, carry/roll return, and the duration-convexity P&L identity. |
| `nimblecas.mmstrat` | [mmstrat.md](reference/mmstrat.md) | Money-market strategies (exact day-count identities): repo interest/repayment (actual/360), discount price ↔ bank-discount-rate, bank-discount → bond-equivalent yield, holding-period return, and the compounded effective rate of a forward-deposit strip. |
