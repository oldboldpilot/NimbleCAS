...
#### Fractional Calculus (Gamma-Based Operators)
Non-integer-order integration and differentiation, built directly on the Gamma function (§7.1) so the order $\alpha$ may be any real (or symbolic) value; every operator collapses to the ordinary derivative/integral at integer $\alpha$ and its results feed back into automatic simplification.
- **Riemann–Liouville fractional integral**: for order $\alpha > 0$ with lower limit $a$,
  $$I_a^\alpha f(x) = \frac{1}{\Gamma(\alpha)}\int_a^x (x-t)^{\alpha-1} f(t)\,dt,$$
  a convolution against the power kernel $\frac{(x-t)^{\alpha-1}}{\Gamma(\alpha)}$ dispatched to the integration engine above; the **semigroup law** $I_a^\alpha I_a^\beta = I_a^{\alpha+\beta}$ is enforced as a simplification identity.
- **Riemann–Liouville derivative**: $D_a^\alpha f = \dfrac{d^n}{dx^n}\, I_a^{\,n-\alpha} f$ with $n = \lceil\alpha\rceil$ — integrate to the fractional order $n-\alpha$, then differentiate an integer $n$ times.
- **Caputo derivative**: the order-swapped ${}^{C}\!D_a^\alpha f = I_a^{\,n-\alpha}\,\dfrac{d^n}{dx^n} f$, which annihilates constants (${}^{C}\!D_a^\alpha c = 0$) and admits ordinary initial conditions — the form used for fractional ODE/IVPs.
- **Grünwald–Letnikov definition**: the limit of a fractional difference quotient,
  $$D^\alpha f(x) = \lim_{h\to 0} h^{-\alpha}\sum_{k\ge 0} (-1)^k \binom{\alpha}{k} f(x-kh),$$
  with the generalized binomial $\binom{\alpha}{k} = \frac{\Gamma(\alpha+1)}{\Gamma(k+1)\,\Gamma(\alpha-k+1)}$ (§7.1, §7.18) — the discretization used on the numerical path.
- **Closed forms**: the power rule $D^\alpha x^p = \dfrac{\Gamma(p+1)}{\Gamma(p-\alpha+1)}\,x^{p-\alpha}$ (and its integral companion $I^\alpha x^p = \dfrac{\Gamma(p+1)}{\Gamma(p+\alpha+1)}\,x^{p+\alpha}$), the exponential $D^\alpha e^{\lambda x} = \lambda^\alpha e^{\lambda x}$, and the **Mittag-Leffler** solutions $E_\alpha(\lambda x^\alpha)$ of the linear fractional ODE ${}^{C}\!D^\alpha y = \lambda y$, all recognized symbolically.

#### Pseudo-Differential Operators
A pseudo-differential operator $P = p(x, D)$ is defined by a **symbol** $p(x,\xi)$ acting through the Fourier transform (§7.3), generalizing constant-coefficient differential operators (whose symbol is a polynomial in $\xi$) to non-polynomial and non-integer-order symbols.
- **Definition**:
...
- **Fractional Laplacian**: the canonical example $(-\Delta)^s$ with symbol $|\xi|^{2s}$, i.e. $\widehat{(-\Delta)^s u}(\xi) = |\xi|^{2s}\,\hat u(\xi)$ — the non-local operator linking pseudo-differential and fractional (above) calculus, with its equivalent hypersingular real-space integral available for the numerical path.
