...
2. **Distributed Hash-Consing**: The cluster implements a distributed memoization key-value table. Simplified sub-expressions are hashed and cached across the network, preventing redundant symbolic simplifications.
   - **Status: BUILT** — [`nimblecas.memo_dist`](reference/memo_dist.md), wired into `SgeeDistributedExecutor` behind two switches that both default off (`dedup_identical_tasks`, `memo`). Keyed on the canonical encoded task, so the registry fingerprint inside it retires stale entries automatically when the code changes. Measured POSITIVE across a 21-cell sweep ([memo-dist-bench.md](technical/memo-dist-bench.md)): the realised fraction of the theoretical saving is governed by task cost alone (~36% at 0.05 ms/task rising to ~99% at 50 ms) and is indifferent to duplicate ratio, over a measured coordinator floor of 17.1 µs per task.
