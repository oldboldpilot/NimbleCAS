...
2. **Distributed Hash-Consing**: The cluster implements a distributed memoization key-value table. Simplified sub-expressions are hashed and cached across the network, preventing redundant symbolic simplifications.
   - **Status: BUILT** — [`nimblecas.memo_dist`](reference/memo_dist.md), wired into `SgeeDistributedExecutor` behind two switches that both default off (`dedup_identical_tasks`, `memo`). Keyed on the canonical encoded task, so the registry fingerprint inside it retires stale entries automatically when the code changes.
