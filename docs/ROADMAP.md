...
- **Financial mathematics** — `bigdecimal` (exact base-10 money type), `finance` (a comprehensive suite: TVM, cashflows, depreciation, bonds, swaps, with a two-tier exact-over-`Q`/numerical honesty contract), `currency` (exact FX rate-graph with BFS cross-rates + triangular-arbitrage detection), `pricing` (Black-Scholes + full & extended Greeks, Kamrad–Ritchken trinomial trees, reproducible Monte Carlo, Longstaff–Schwartz American, Black-76/digitals/barriers, SVG plotting), and `analytics` + `portfolio` (Sharpe/Sortino/Treynor/Jensen-α/β, historical + parametric VaR/CVaR, ridge-regularized mean-variance optimization). All untrusted numeric/string entry points are DoS-bounded (see [finance capability roadmap](technical/finance-parity-roadmap.md)); worked examples in the [financial-mathematics guide](guides/financial-mathematics.md).
