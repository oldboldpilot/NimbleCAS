...
3. **Data Locality Optimization**: Tasks are scheduled on nodes that already hold parent datasets (e.g. a downstream matrix calculation is scheduled on the specific GPU holding the parent matrix) to minimize inter-node TCP/IP or PCIe transfers.
   - **Status: BLOCKED at the C ABI, not in NimbleCAS.** The coordinator-side half already exists (`SgeeExecutorConfig::placement` is consulted at both enqueue sites, and `taskdag_sched` has `Affinity`/`to_placement` ready). The pull side does not: `sgee_broker_lease` hands out the earliest pending task and accepts no locality key or placement filter, so a worker cannot request the work whose data it already holds. SGEE upstream has since added `LeaseRequest.payload_filter` in its core; exposing that through the C ABI is the unblocking change.
