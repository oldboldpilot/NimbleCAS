...
**Done (built on the clang + libc++ server, `ctest` green, ASan/UBSan clean, per-module review):**
...
- **Reasoning & algorithmics** — a complete layer, not a sampler. `search` (BFS/DFS/IDDFS/Dijkstra/A*/IDA*/weighted-A*/greedy/beam/bidirectional/tabu plus edit-distance/LCS/knapsack DP), `sat`, `csp`, and the branchless `bitset`/`bitcsp`, on the `parallel` fork–join runtime. **Prolog**: `logic` is a complete solver (cut with barrier semantics, the control constructs, arithmetic, all-solutions predicates, an assertable database), with `logic_parser` reading real Prolog text and `logic_index` giving the database first-argument indexing rather than a linear scan. **Ahead-of-time emission**: `logic_compile` compiles a program to C++23, CUDA and Triton at three integer widths using continuation-passing, so nondeterministic predicates compile and not merely the deterministic subset; `sat_compile` emits a satisfiability solver as source — complete bit-parallel enumeration (honest only to about thirty variables, since it is 2^n) and the WalkSAT family (`skc`, `probsat`, `novelty_plus`, `adaptive_novelty_plus`) for scale, with CDCL deliberately NOT emitted because clause learning is a sequential dependency chain. **Distribution over `taskdag`**: `logic_dist` (SLD resolution), `search_dist` (round-based frontier relaxation under `min_plus` and `max_min` semirings — SSSP/A*/BFS/Floyd–Warshall/MST/k-core/triangles/SCC), and `sat_dist` (portfolio, plus cube-and-conquer partitioning the assignment space exactly, so a cluster-wide UNSAT proof is possible at all). **Constraints**: `csp` gains a declarative `WireCsp` form -- eight constraint kinds as pure data -- because a CSP whose constraints are `std::function` closures can be solved in process but can be neither shipped to a worker nor read by a code generator; `as_csp` converts back, so the same problem runs through every existing solver and the distributed and emitted answers are checkable against a known-good one. On that rest `csp_compile`, which emits C++23, CUDA and Triton (complete exhaustive enumeration, and `min_conflicts` local search that reports UNKNOWN and never unsatisfiability; backtracking is deliberately NOT emitted, pruning being a sequential dependency chain), and `csp_dist`, whose prefix decomposition partitions the assignment space exactly -- so a cluster-wide UNSAT proof is possible, and the per-prefix counts sum to the total, which is distributed model counting rather than merely distributed search. **Planning**: `planning_dist` distributes it over `taskdag` -- the coordinator owns the open and closed lists and decides what to expand, workers expand and evaluate and own nothing, and A* takes a whole f-layer per round, which is a legal A* expansion order and so keeps optimality. The contract is stated exactly rather than conveniently: the same COST as `astar_plan` always, not necessarily the same plan, since `astar_plan` breaks ties toward the state discovered earlier and a parallel layer has no such order. **Planning**: `planning` is SAS+ rather than STRIPS, so mutual exclusion is structural, with `h_max` admissible, `h_add`/`h_ff` not, and every returned plan replayed from the initial state before it is returned. The distributed modules are verified over `SgeeDistributedExecutor` itself — real payload encoding, a broker queue, the worker's own registry lookup — though the transport in those tests is an in-process broker, so what is established is the executor, wire format and registry contract, not a real cluster under partition.
...
- **Financial mathematics** — `bigdecimal` (exact base-10 money type), `finance` (a comprehensive suite: TVM, cashflows, depreciation, bonds, swaps, with a two-tier exact-over-`Q`/numerical honesty contract), `currency` (exact FX rate-graph with BFS cross-rates + triangular-arbitrage detection), `pricing` (Black-Scholes + full & extended Greeks, Kamrad–Ritchken trinomial trees, reproducible Monte Carlo, Longstaff–Schwartz American, Black-76/digitals/barriers, SVG plotting), and `analytics` + `portfolio` (Sharpe/Sortino/Treynor/Jensen-α/β, historical + parametric VaR/CVaR, ridge-regularized mean-variance optimization). Extended by `exotics` (CRR + discrete divs, Reiner-Rubinstein barriers, lookback, Crank-Nicolson PDE, Margrabe/Kirk/Geske/chooser/basket), `yieldcurve` (pillar `Curve`, conversions, bootstrapping, Nelson-Siegel/Svensson, a Hull-White short-rate lattice with callable-bond pricing + option-adjusted spread), `fixedincome` (DV01/PV01, key-rate durations, cfamounts, odd-period pricing, z-spread, FRN/amortization), and `riskextra` (constrained-frontier active-set QP, LPM, EW moments, Rockafellar-Uryasev CVaR, French depreciation). The counter-based RNG feeding the Monte Carlo engines has an AVX-512 batched core (~8× on the RNG hot path). All untrusted numeric/string entry points are DoS-bounded (see [finance capability roadmap](technical/finance-parity-roadmap.md)); worked examples in the [financial-mathematics guide](guides/financial-mathematics.md).
...
- **§6.2 item 1 is built; the rest of §6 is not.** `taskdag` + `taskdag_sgee` carry a task DAG in-process, across processes over gRPC, and across a **3-node Raft quorum with mTLS and leader failover** — outputs bit-identical to `serial_executor()`, every transport failure an honest `distributed_error`. On top of that, **`modgcd` + `modgcd_dist` distribute a real computation**: Brown's modular GCD for univariate Z[x], one image task per prime, CRT-merged and verified by exact trial division, byte-identical whether it runs serially or across the quorum. **§6.2 item 2 is now built too**: `memo_dist` supplies a process-stable 128-bit `ContentKey` over the canonical encoded task — deliberately not `Expr::structural_hash()`, whose `std::hash` values differ across standard libraries and so cannot address content across a cluster — and the coordinator consults it before dispatch and publishes to it after collection, so a task whose bytes are already known is never sent. Every hit is confirmed by a full-key byte comparison, making a fingerprint collision a lost cache entry rather than a wrong answer. Still open: **§6.1's stochastic scheduling** was measured and came out NEGATIVE (it ships off by default), and **§6.2 item 3, data-locality scheduling**, has its coordinator half built (M8) but cannot route: `sgee_broker_lease` selects the earliest pending task, takes no locality key, and nothing reports which worker produced a result, so the label a task carries is inert. The two required C ABI additions are specified in [sgee-locality-abi.md](technical/sgee-locality-abi.md). Multivariate GCD is out of scope for now (no multivariate polynomial representation exists). The result store is **no longer only in-memory**: Raft still replicates the queue rather than results, but a file-backed `DistributedMemo` ([`memo_dist`](reference/memo_dist.md)) makes results outlive the coordinator, so a run repeated after a restart dispatches nothing and returns bit-identical outputs. Durability is bounded and documented: records survive the **process** dying, but not power loss — `std::fstream` exposes no descriptor to `fsync`, so that is honestly recorded as not implemented rather than offered behind a flag that could not deliver it. A damaged tail is detected, cut at open, and treated as a miss, never as data. Beyond the file, **[`memo_postgres`](reference/memo_postgres.md) puts the memo in a PostgreSQL table**, which is what a cluster actually needs: `InProcessMemo` is one process and `FileMemo` is one filesystem, and executors on different machines share neither. It is OPT-IN behind `NIMBLECAS_POSTGRES` and is the only module in the repository with a third-party runtime dependency (libpq), the build being dependency-free by policy everywhere else. Its honesty boundary runs opposite to the convenient direction: every transport failure is `distributed_error` and NEVER a miss, because reporting an unreachable database as a cache miss would let a run silently recompute everything while looking healthy. A fingerprint collision returns a miss and never the colliding value, verified against a live server rather than only compiled.
...
2. **Distributed Hash-Consing**: The cluster implements a distributed memoization key-value table. Simplified sub-expressions are hashed and cached across the network, preventing redundant symbolic simplifications.
   - **Status: BUILT** — [`nimblecas.memo_dist`](reference/memo_dist.md), wired into `SgeeDistributedExecutor` behind two switches that both default off (`dedup_identical_tasks`, `memo`). Keyed on the canonical encoded task, so the registry fingerprint inside it retires stale entries automatically when the code changes. Measured POSITIVE across a 21-cell sweep ([memo-dist-bench.md](technical/memo-dist-bench.md)): the realised fraction of the theoretical saving is governed by task cost alone (~35% at 0.05 ms/task rising to ~99% at 50 ms) and is indifferent to duplicate ratio, over a measured coordinator floor of 17.1 µs per task.
3. **Data Locality Optimization**: Tasks are scheduled on nodes that already hold parent datasets (e.g. a downstream matrix calculation is scheduled on the specific GPU holding the parent matrix) to minimize inter-node TCP/IP or PCIe transfers.
   - **Status: coordinator side BUILT (M8); routing BLOCKED at the C ABI, not in NimbleCAS.** `taskdag_sgee::affinity_placement` now builds the `placement` function from an `AffinityTable`, so every task is labelled and the label travels coordinator → broker → worker through the ABI's one `int placement` per task. `Affinity`/`to_placement` are no longer dead code. **Turning it on changes nothing today, and it says so**: `sgee_broker_lease` hands out the earliest pending task and accepts no locality key or filter, so no worker acts on the label. Two ABI additions are needed — a filtered lease (already in SGEE's core, proto, server and client since `28537cc5`; only the C shim is missing, an estimated 130-160 additive lines) and a way to learn which worker produced a result (`worker_id` is currently only ever an *input* to lease, so the coordinator cannot know where a parent's data lives). Both are specified in [sgee-locality-abi.md](technical/sgee-locality-abi.md).
...
- **Complex Numbers**:
  - **Representation**: A `ComplexNode` holding the Cartesian pair $z = a + b i$, with $a, b$ kept as exact `Rational` (via `nimblecas.ratpoly`) whenever the parts are exact and as arbitrary symbolic sub-trees otherwise, so $\mathbb{Q}(i)$ arithmetic stays exact and Gaussian-rational results never drift to floating point.
  - **Field arithmetic**: Automatic simplification implements the complex field operations — addition/subtraction componentwise, multiplication $(a+bi)(c+di) = (ac-bd) + (ad+bc)i$, and division by rationalizing with the conjugate $\frac{z}{w} = \frac{z\bar w}{|w|^2}$ — canonicalizing to the $a + bi$ normal form.
  - **Conjugate, modulus, argument**: the complex conjugate $\bar z = a - b i$, modulus $|z| = \sqrt{a^2 + b^2}$ (exact surd where possible), and argument $\arg z = \operatorname{atan2}(b, a)$ taken on the principal branch $(-\pi, \pi]$.
  - **Polar / exponential form**: conversion to $z = r e^{i\theta} = r(\cos\theta + i\sin\theta)$ via **Euler's formula** $e^{i\theta} = \cos\theta + i\sin\theta$, with De Moivre's theorem $z^n = r^n(\cos n\theta + i\sin n\theta)$ used for integer powers.
  - **Roots & roots of unity**: the $n$ distinct $n$-th roots $z^{1/n} = r^{1/n}\exp\!\big(i\frac{\theta + 2\pi k}{n}\big)$ for $k = 0,\dots,n-1$, and the primitive **roots of unity** $\omega_k = e^{2\pi i k/n}$ (feeding the FFT twiddle factors of §7.3).
  - **Complex elementary functions**: $\exp$, $\log$ with explicit **branch-cut** bookkeeping (principal branch $\operatorname{Log} z = \ln|z| + i\arg z$ with a cut along the negative real axis), complex trigonometric/hyperbolic functions (related through $\cos z, \sin z$ and $\cosh z, \sinh z$), and complex powers $z^w = e^{w \operatorname{Log} z}$ (multivalued, principal-branch by default).
  - **Identities**: automatic simplification handles branch cuts, conjugation $\overline{z_1 z_2} = \bar z_1 \bar z_2$, Euler's-formula conversion ($e^{i\theta} \to \cos\theta + i\sin\theta$), and residue tagging feeding the Laurent-series/residue machinery of §7.3.

#### Trigonometric Identities
An identity-driven layer that extends automatic simplification (§2.2) with symbolic recognition and application of the standard trigonometric identities — used both to **normalize/simplify** trig expressions into canonical form and to **enable trig integrals** (feeding the table-driven integration layer of §7.19). Each identity is stored for fast speculative recall behind the shared identity/pattern filter (§9).
- **Pythagorean**: $\sin^2 x + \cos^2 x = 1$ and its derived forms $1 + \tan^2 x = \sec^2 x$, $1 + \cot^2 x = \csc^2 x$.
- **Angle-sum / difference**: $\sin(x\pm y) = \sin x\cos y \pm \cos x\sin y$, $\cos(x\pm y) = \cos x\cos y \mp \sin x\sin y$, $\tan(x\pm y) = \frac{\tan x \pm \tan y}{1 \mp \tan x\tan y}$.
- **Double-, half-, and multiple-angle**: $\sin 2x = 2\sin x\cos x$, $\cos 2x = \cos^2 x - \sin^2 x = 1 - 2\sin^2 x$, the half-angle forms $\sin^2\frac{x}{2} = \frac{1-\cos x}{2}$, and multiple-angle expansions via De Moivre / Chebyshev ($\cos nx = T_n(\cos x)$, §7.16).
- **Power-reduction**: $\sin^2 x = \frac{1-\cos 2x}{2}$, $\cos^2 x = \frac{1+\cos 2x}{2}$, and higher powers reduced to first-degree harmonics (the key rewrite for integrating powers of sine/cosine).
- **Product-to-sum & sum-to-product**: $2\sin x\cos y = \sin(x+y) + \sin(x-y)$, $\sin x + \sin y = 2\sin\frac{x+y}{2}\cos\frac{x-y}{2}$, and companions — used to linearize products and collapse sums.
- **Tangent half-angle (Weierstrass) substitution**: $t = \tan\frac{x}{2}$ giving $\sin x = \frac{2t}{1+t^2}$, $\cos x = \frac{1-t^2}{1+t^2}$, $dx = \frac{2}{1+t^2}\,dt$ — the canonical rationalizing substitution that turns any rational trig integrand into a rational function for the Hermite/Rothstein–Trager engine (§7.19).

#### Complex Analysis
Building on the `ComplexNode` field above, the engine treats holomorphic-function theory as a first-class analytical branch that reuses symbolic differentiation (§7.19), Laurent-series expansion and residue extraction (§7.3), and the contour/integration substrate of §7.19.
- **Cauchy's integral theorem & formula**: for $f$ holomorphic on and inside a closed contour $\gamma$, $\oint_\gamma f(z)\,dz = 0$, and the Cauchy integral formula $f^{(n)}(a) = \frac{n!}{2\pi i}\oint_\gamma \frac{f(z)}{(z-a)^{n+1}}\,dz$ recovers derivatives and Taylor coefficients from a contour.
- **Contour integration**: parameterized path integrals $\int_\gamma f\,dz$ over line segments, circular arcs, and keyhole/semicircular contours, with the **ML (estimation) inequality** bounding tail contributions as radii grow.
- **Laurent series & residue calculus**: Laurent expansion $f(z) = \sum_{k=-\infty}^{\infty} c_k (z-a)^k$ about an isolated singularity (§7.3), with the **residue** $\operatorname{Res}(f, a) = c_{-1}$ read off directly or via the pole-order formula $\operatorname{Res}(f,a) = \frac{1}{(m-1)!}\lim_{z\to a}\frac{d^{m-1}}{dz^{m-1}}\big[(z-a)^m f(z)\big]$.
- **Residue theorem applications**: $\oint_\gamma f\,dz = 2\pi i \sum_k \operatorname{Res}(f, a_k)$ used as an automated engine for evaluating **real definite integrals** (e.g. $\int_{-\infty}^{\infty}$ rational and rational$\times$trig integrands, $\int_0^{2\pi}$ trigonometric integrals via the $z = e^{i\theta}$ substitution) and **infinite sums** (through the $\pi\cot(\pi z)$ / $\pi\csc(\pi z)$ summation kernels, connecting to §7.3).

#### Blaschke Products
Bounded analytic self-maps of the unit disk, used as the **inner functions** in factorizations of bounded holomorphic functions and as building blocks for the residue/contour machinery above.
- **Finite Blaschke products**: for zeros $a_k$ in the open unit disk $|a_k| < 1$,
  $$B(z) = \prod_k \frac{|a_k|}{a_k}\,\frac{a_k - z}{1 - \overline{a_k}\,z},$$
  a rational holomorphic map with $|B(z)| = 1$ on the boundary $|z| = 1$ (the unimodular prefactor $|a_k|/a_k$ normalizes $B(0) > 0$); each factor is a disk automorphism (Möbius transformation of the disk).
- **Infinite Blaschke products**: the infinite product $B(z) = \prod_k \frac{|a_k|}{a_k}\,\frac{a_k - z}{1 - \overline{a_k}\,z}$ converges to a bounded analytic function on the disk **iff** the zeros satisfy the **Blaschke condition** $\sum_k (1 - |a_k|) < \infty$ (verified through the $\log$-to-series infinite-product convergence test of §7.3).
- **Role**: represented as a dedicated inner-function object carrying its zero multiset, Blaschke products are the canonical **inner factors** ($|B| = 1$ a.e. on the boundary) in the inner–outer factorization of bounded analytic ($H^\infty$) functions on the disk, and supply exact unimodular test functions for the contour-integration engine.

#### Riemann Zeta Function
A dedicated `ZetaNode` special function reusing the infinite-series/convergence engine (§7.3), the Bernoulli-number machinery (§7.18), and the complex-analytic continuation apparatus above.
- **Series & Euler product**: the Dirichlet series $\zeta(s) = \sum_{n\ge 1} n^{-s}$ for $\Re(s) > 1$, equal to the **Euler product** over primes $\zeta(s) = \prod_{p}\big(1 - p^{-s}\big)^{-1}$, which encodes the fundamental theorem of arithmetic and anchors analytic number theory.
- **Analytic continuation & functional equation**: continuation to $\mathbb{C}\setminus\{1\}$ (a simple pole at $s=1$ with residue $1$) via the **functional equation**
  $$\zeta(s) = 2^s \pi^{s-1} \sin\!\Big(\frac{\pi s}{2}\Big)\,\Gamma(1-s)\,\zeta(1-s),$$
  relating values at $s$ and $1-s$ across the critical strip.
- **Closed forms at integers**: even positive integers via Bernoulli numbers (cross-reference §7.18),
  $$\zeta(2n) = \frac{(-1)^{n+1} B_{2n} (2\pi)^{2n}}{2\,(2n)!} \quad(\text{e.g. } \zeta(2) = \tfrac{\pi^2}{6},\ \zeta(4) = \tfrac{\pi^4}{90}),$$
  and negative integers $\zeta(-n) = -\frac{B_{n+1}}{n+1}$, yielding the **trivial zeros** $\zeta(-2n) = 0$ at the negative even integers.
- **Critical line & Riemann Hypothesis**: the non-trivial zeros lie in the critical strip $0 < \Re(s) < 1$; the **Riemann Hypothesis** conjectures they all lie on the critical line $\Re(s) = \tfrac12$ — carried as documented context/metadata for numerical zero-location experiments rather than assumed.
- **Related $L$-functions**: the **Dirichlet eta** (alternating zeta) $\eta(s) = \sum_{n\ge1}(-1)^{n-1} n^{-s} = (1 - 2^{1-s})\zeta(s)$, which converges for $\Re(s) > 0$ and provides a continuation vehicle, and the general **Dirichlet $L$-function** $L(s,\chi) = \sum_{n\ge1}\chi(n) n^{-s}$ with its own Euler product over primes — the natural generalization of $\zeta$.
...
Every `MatrixNode` (backed by the already-implemented `nimblecas.matrix` module of exact-`Rational` dense matrices) carries a **structure tag** detected at construction (or asserted by the caller) so that downstream operations select a structure-aware fast path and compact storage rather than treating the matrix as dense-general.
- **Symmetry classes** (over $\mathbb{R}$ or, via the complex field of §7.1, over $\mathbb{C}$): **symmetric / Hermitian** $A = A^\dagger$, **skew-symmetric / skew-Hermitian** $A = -A^\dagger$, **orthogonal / unitary** $A^\dagger A = I$, **normal** $A A^\dagger = A^\dagger A$, and **positive-(semi)definite** (verified by the Cholesky / $LDL^\dagger$ test below).
- **Bandedness / triangularity**: **diagonal**, **bidiagonal**, **tridiagonal**, general **banded** with lower/upper bandwidth $(p, q)$, **upper/lower triangular**, and **upper/lower Hessenberg** (one sub-/super-diagonal off triangular).
- **Displacement / combinatorial structure**: **Toeplitz** (constant diagonals), **Hankel** (constant anti-diagonals), **circulant** (cyclic Toeplitz), **Vandermonde**, **permutation**, **block / block-diagonal**, and **sparse** (compressed storage feeding the iterative solvers above).
- **Storage & dispatch**: the tag selects packed storage (only the band, the generating vector of a Toeplitz/circulant, the diagonal, …) and routes to the structure-aware algorithms of the fast-algorithm subsection below; Hermitian/unitary tags additionally carry the complex-conjugate bookkeeping of §7.1.

#### Matrix Decompositions
A unified factorization layer, structure-tag aware (a symmetric positive-definite input dispatches to Cholesky, a Hermitian input to a real-spectrum eigensolver, and so on):
- **LU**: Gaussian elimination with **partial** (row) or **full** (row+column) pivoting, $PA = LU$ (or $PAQ = LU$), driving general solves, determinants, and inverses.
- **Cholesky & $LDL^\dagger$**: for Hermitian/symmetric positive-definite $A = LL^\dagger$ (the Cholesky bullet above) and the pivoted, square-root-free $A = LDL^\dagger$ for indefinite Hermitian systems.
- **QR**: **Householder** reflections (backward-stable default), **Givens** rotations (structure-preserving for Hessenberg/banded matrices and updating), and **modified Gram–Schmidt**, $A = QR$.
- **Hessenberg / tridiagonal reduction**: orthogonal/unitary reduction of a general matrix to **upper Hessenberg** (and a Hermitian matrix to **tridiagonal**) form as the condensation step before the QR eigenvalue iteration.
- **Eigen-decomposition**: real-spectrum solvers for Hermitian/symmetric matrices (**Jacobi rotations** or the **symmetric / tridiagonal QR** algorithm) and, for general matrices, the **Francis double-shift QR** iteration on Hessenberg form, extending the dense-QR / Lanczos / Arnoldi solvers already listed.
- **Schur, Jordan & spectral**: the **Schur decomposition** $A = QTQ^\dagger$ (unitary $Q$, upper-triangular $T$), the **Jordan canonical form** $A = PJP^{-1}$ for the exact defective/symbolic case, and the **spectral decomposition** $A = \sum_i \lambda_i P_i$ for normal matrices.
- **SVD & polar**: the **singular value decomposition** $A = U\Sigma V^\dagger$ and the **polar decomposition** $A = UP$ (unitary factor $\times$ Hermitian positive-semidefinite factor), related through the SVD.

> **Realized** across the canonical-form / decomposition modules: [`qrschur`](reference/qrschur.md) (exact Gram–Schmidt orthogonal QR over `Q` + numeric Householder QR and Francis real-**Schur** form), [`frobenius`](reference/frobenius.md) (**rational canonical form** exactly over `Q` via the Smith normal form of `xI − A`), [`jordan`](reference/jordan.md) (**Jordan form** `A = PJP⁻¹` with the transforming `P`, exact over `Q` and over a quadratic extension `Q(α)` built on [`algnum`](reference/algnum.md)), [`cheigen`](reference/cheigen.md) (eigenvalues of complex/Hermitian matrices), and [`kronprod`](reference/kronprod.md) (**Kronecker/direct/Hadamard** products with the `vec` identities). Full SVD/polar and the general splitting-field Jordan (degree ≥ 3) remain future work.

#### Matrix Functions, Invariants & Transforms
- **Matrix functions**: the **matrix exponential** $e^{A}$ by **scaling-and-squaring with a Padé approximant** (§7.5) or by an **eigen- / Schur–Parlett** evaluation, the **logarithm** $\log A$, **square root** $A^{1/2}$, and general **$f(A)$** via the Schur–Parlett recurrence; integer and real **powers** $A^k$, the **inverse** $A^{-1}$, and the **Moore–Penrose pseudoinverse** $A^+$ (from the SVD).
- **Scalar invariants**: **determinant**, **trace**, **rank**, and **condition number** $\kappa(A) = \|A\|\,\|A^{-1}\|$.
- **Norms**: the induced $1$-, $2$- (spectral, $= \sigma_{\max}$), and $\infty$-norms, the **Frobenius** norm $\|A\|_F = \sqrt{\sum_{ij}|a_{ij}|^2}$, and the **nuclear** norm $\|A\|_* = \sum_i \sigma_i$.
- **Products & sums of spaces**: the **Kronecker / tensor product** $A \otimes B$ and the **direct sum** $A \oplus B$, with the identities $(A\otimes B)(C\otimes D) = AC \otimes BD$ and $\operatorname{vec}(AXB) = (B^\top \otimes A)\operatorname{vec}(X)$ used to linearize matrix (Sylvester/Lyapunov) equations.
- **Characteristic apparatus**: the **characteristic polynomial** $p_A(\lambda) = \det(\lambda I - A)$ and the **minimal polynomial**, the **Cayley–Hamilton theorem** $p_A(A) = 0$ (used to reduce high powers and express $A^{-1}$ / $f(A)$ as a polynomial in $A$), and the **adjugate** $\operatorname{adj}(A)$ with $A\,\operatorname{adj}(A) = \det(A)\,I$.
- **Similarity & congruence**: **similarity** transforms $A \mapsto P^{-1}AP$ (preserving the spectrum, underlying the eigen / Jordan / Schur forms) and **congruence** transforms $A \mapsto P^\dagger A P$ (preserving inertia, Sylvester's law).

#### Structure-Aware Fast Algorithms
Recognized structure (above) is exploited for asymptotically faster, more stable kernels:
- **Triangular solves**: forward / back substitution in $O(n^2)$ for the triangular factors of LU / QR / Cholesky.
- **Banded factorizations**: banded LU / Cholesky in $O(n\,pq)$ / $O(n p^2)$, and the **Thomas algorithm** (§7.14) for the tridiagonal case.
- **Toeplitz solvers**: the **Levinson–Durbin** recursion in $O(n^2)$ (and a superfast $O(n\log^2 n)$ variant), used directly for the **Yule–Walker** equations of AR time-series estimation (§7.8.4).
- **Circulant via FFT**: circulant matrices are diagonalized by the DFT, $C = F^{-1}\operatorname{diag}(\hat c)\,F$, so mat-vec products and linear solves run in $O(n\log n)$ through the FFT layer (§7.3) — the same primitive underlying fast Toeplitz mat-vecs by circulant embedding.
- **Hessenberg QR iteration**: eigenvalue iteration on the condensed Hessenberg / tridiagonal form costs $O(n^2)$ per sweep instead of the $O(n^3)$ of a dense sweep.
- **Hermitian / unitary exploitation**: Hermitian structure guarantees a **real spectrum** and orthogonal eigenvectors, enabling the stable symmetric eigensolvers above; unitary structure gives $A^{-1} = A^\dagger$ for free.

#### Tensor Analysis
Tensors generalize the rank-2 **matrices** of §7.2 and rank-1 **vectors** to arbitrary rank/order; the algebraic layer lives here alongside the matrix engine, while the differential-geometric operators extend the vector-calculus module of §7.19.
- **Tensors & tensor algebra**: a `TensorNode` holding multilinear components with tracked **covariant (lower)** and **contravariant (upper)** index slots of arbitrary rank, contracted under the **Einstein summation convention** (a repeated upper/lower index pair is summed implicitly). The **metric tensor** $g_{ij}$ and its inverse $g^{ij}$ ($g^{ik}g_{kj} = \delta^i_j$) **raise and lower** indices ($v^i = g^{ij}v_j$), with the **Kronecker delta** $\delta^i_j$ and the **Levi-Civita symbol** $\varepsilon_{ijk\ldots}$ as built-in invariant tensors. Core operations: the **outer (tensor) product** $\otimes$ (rank-additive), **contraction** over an upper/lower pair (rank-reducing by two; the **trace** $T^i{}_i$ is the rank-2 case), and the **symmetrization** $T_{(ij)}$ / **antisymmetrization** $T_{[ij]}$ projectors.
- **Tensor calculus (differential geometry)**: extends the vector-calculus operators (§7.19) to curved/curvilinear coordinates. The **covariant derivative** $\nabla_i$ corrects the partial derivative with **Christoffel symbols** $\Gamma^k_{ij} = \tfrac12 g^{kl}\big(\partial_i g_{jl} + \partial_j g_{il} - \partial_l g_{ij}\big)$ derived symbolically from the metric (so $\nabla_i v^k = \partial_i v^k + \Gamma^k_{ij}v^j$), driving **parallel transport** and the **geodesic** equation $\ddot{x}^k + \Gamma^k_{ij}\dot{x}^i\dot{x}^j = 0$. Curvature is assembled as the **Riemann tensor** $R^\rho{}_{\sigma\mu\nu} = \partial_\mu\Gamma^\rho_{\nu\sigma} - \partial_\nu\Gamma^\rho_{\mu\sigma} + \Gamma^\rho_{\mu\lambda}\Gamma^\lambda_{\nu\sigma} - \Gamma^\rho_{\nu\lambda}\Gamma^\lambda_{\mu\sigma}$, contracting to the **Ricci tensor** $R_{\mu\nu} = R^\rho{}_{\mu\rho\nu}$, the **scalar curvature** $R = g^{\mu\nu}R_{\mu\nu}$, and the **Einstein tensor** $G_{\mu\nu} = R_{\mu\nu} - \tfrac12 R\,g_{\mu\nu}$. The curvilinear operators generalize §7.19: gradient $\nabla_i f$, divergence $\nabla_i v^i = \tfrac{1}{\sqrt{g}}\partial_i(\sqrt{g}\,v^i)$, curl (via $\varepsilon$), and the **Laplace–Beltrami** operator $\Delta f = \tfrac{1}{\sqrt{g}}\partial_i\big(\sqrt{g}\,g^{ij}\partial_j f\big)$.
- **Differential forms & exterior calculus**: antisymmetric covariant tensors as **$p$-forms**, combined by the **wedge product** $\wedge$ (graded-anticommutative, $\alpha\wedge\beta = (-1)^{pq}\beta\wedge\alpha$), differentiated by the **exterior derivative** $d$ (raising a $p$-form to a $(p{+}1)$-form, with the nilpotency $d^2 = 0$), and dualized by the metric-dependent **Hodge star** $\star$. The **generalized Stokes theorem** $\int_M d\omega = \int_{\partial M}\omega$ unifies the Green / Stokes / divergence theorems already recorded under §7.19's multiple integrals (each is a low-dimensional instance), and $d^2 = 0$ together with $\star$ recovers the curl-of-gradient and divergence-of-curl identities.
...
- **Products of Series**: the **Cauchy product** of two series $\left(\sum_n a_n\right)\left(\sum_n b_n\right) = \sum_n c_n$ has coefficients $c_n = \sum_{k=0}^{n} a_k b_{n-k}$ (a discrete convolution, dispatched to the SIMD/GPU coefficient-multiply path of §5.3 for high degree, Karatsuba above a tuned crossover). Convergence of the product series is certified by **Mertens' theorem** — if $\sum a_n$ and $\sum b_n$ both converge and **at least one converges absolutely**, then $\sum c_n$ converges to the product of the two sums — with Abel's theorem covering the boundary case when only Cesàro/Abel summability is available. This is the multiplicative core of **power-series multiplication and composition**: composing $f(g(z))$ propagates coefficients through the same convolution, and for algebraic/differential relations the product's coefficients are captured as a **recurrence** $c_n = \dots$ (a difference equation, §7.9) so the whole product series is generated term-by-term without re-convolving.
- **Infinite Series & Convergence Tests**: Given an infinite series $\sum_{n} a_n$, the engine decides convergence/divergence with a full battery of symbolic tests, applied in a heuristic waterfall (cheapest/most decisive first) and each returning a verdict with the witnessing quantity (limit, ratio, comparison series, …):
  - **$n$-th-term (divergence) test**: if $\lim_{n\to\infty} a_n \ne 0$ the series diverges (a necessary-condition screen run first).
  - **Geometric & $p$-series**: closed verdicts for $\sum r^n$ (converges iff $|r| < 1$, to $\frac{a}{1-r}$) and $\sum n^{-p}$ (converges iff $p > 1$), used as the canonical comparison yardsticks.
  - **Comparison & limit-comparison**: bound $a_n$ against a known series, or test $\lim_{n\to\infty} a_n/b_n \in (0,\infty)$ for like-behaved $b_n$.
  - **Ratio (d'Alembert)** $L = \lim_{n\to\infty}\left|\frac{a_{n+1}}{a_n}\right|$ and **root (Cauchy)** $L = \limsup_{n\to\infty}\sqrt[n]{|a_n|}$: converge for $L < 1$, diverge for $L > 1$, inconclusive at $L = 1$.
  - **Integral test**: for eventually positive-decreasing $a_n = f(n)$, tie $\sum a_n$ to $\int^\infty f(x)\,dx$ via the symbolic integration engine (§7.19).
  - **Alternating (Leibniz)**: $\sum (-1)^n b_n$ with $b_n \downarrow 0$ converges, with the tail bound $|R_N| \le b_{N+1}$; plus **Dirichlet** and **Abel** tests for $\sum a_n b_n$ with bounded partial sums / monotone factors.
  - **Raabe** $\lim_{n\to\infty} n\big(\frac{a_n}{a_{n+1}} - 1\big)$ and **Cauchy condensation** $\sum a_n \leftrightarrow \sum 2^n a_{2^n}$: sharper tests for the $L=1$ boundary cases (e.g. $\sum \frac1{n(\ln n)^p}$).
  - **Absolute vs conditional convergence**: classify $\sum |a_n|$ separately; conditionally convergent series carry a **Riemann rearrangement** caveat (terms can be reordered to any sum), so reordering rewrites are blocked unless absolute convergence is established.
  - **Radius / interval of convergence**: for power series $\sum a_n (z-z_0)^n$, the radius $R = 1/\limsup_n \sqrt[n]{|a_n|}$ (Cauchy–Hadamard, consistent with the Laurent/Taylor radius above) with explicit endpoint testing to fix the open/closed **interval of convergence** on $\mathbb{R}$.
- **Infinite Products**: convergence of $\prod_n (1 + a_n)$ is decided by the **$\log$-to-series transform** $\log\prod_n(1+a_n) = \sum_n \log(1+a_n)$, reducing to the series tests above; the product converges (to a non-zero limit) iff $\sum_n |a_n| < \infty$ (absolute convergence of the associated series). Blaschke-product convergence (§7.1) reuses exactly this criterion.
- **Infinite-Product Identities**: a catalog of canonical closed-form product identities the engine **recognizes and rewrites** (in either direction), each verified against the convergence criterion above:
  - **Euler product for sine**: $\sin(\pi z) = \pi z \prod_{n\ge1}\big(1 - \frac{z^2}{n^2}\big)$ — the factorization over the integer zeros of $\sin$, underlying the $\zeta(2n)$ closed forms (§7.1).
  - **Wallis product**: $\frac{\pi}{2} = \prod_{n\ge1}\frac{4n^2}{4n^2 - 1} = \prod_{n\ge1}\frac{2n}{2n-1}\cdot\frac{2n}{2n+1}$.
  - **Viète's formula**: $\frac{2}{\pi} = \prod_{n\ge1}\cos\!\big(\frac{\pi}{2^{n+1}}\big) = \frac{\sqrt2}{2}\cdot\frac{\sqrt{2+\sqrt2}}{2}\cdots$ (nested-radical form).
  - **Gamma reflection & Weierstrass product**: the reflection formula $\Gamma(z)\Gamma(1-z) = \frac{\pi}{\sin(\pi z)}$ (a direct consequence of the sine product), and the Weierstrass product for the reciprocal Gamma $\frac{1}{\Gamma(z)} = z\,e^{\gamma z}\prod_{n\ge1}\big(1 + \frac{z}{n}\big)e^{-z/n}$ (with the Euler–Mascheroni constant $\gamma$).
  - **Euler product for $\zeta$**: $\zeta(s) = \prod_{p}\big(1 - p^{-s}\big)^{-1}$ over primes (cross-reference the Riemann-zeta subsection, §7.1).
...
- **Fourier Convolution Theorem**: the transform of a convolution is the pointwise product of transforms, $\widehat{f * g} = \hat f\,\hat g$, with the **dual** (multiplication $\leftrightarrow$ convolution) relation $\widehat{f\cdot g} = \frac{1}{2\pi}\,\hat f * \hat g$. This yields **fast convolution** in $O(N\log N)$ — both linear and circular convolution — by transforming, multiplying, and inverting through the FFT layer, reusing the multi-dimensional (separable) FFT above for image/field convolutions and the pseudo-spectral non-linear products of the spectral solvers. The discrete **Cauchy product** of two coefficient sequences ($c_n = \sum_k a_k b_{n-k}$, above) is exactly this convolution in the polynomial/series setting, unifying series multiplication with the transform convolutions here (and dispatched to the same FFT-based fast path for high degree).
- **Power Spectral Density (PSD)**: for a stationary process the PSD is the Fourier transform of the autocovariance (the **Wiener–Khinchin theorem**) $S(\omega) = \mathcal{F}\{\gamma(h)\} = \sum_h \gamma(h)\,e^{-i\omega h}$ (cross-reference the time-series autocovariance/ACF of §7.7.2 and §7.8.4), with the **Parseval/Plancherel** energy identity $\int |f|^2\,dx = \frac{1}{2\pi}\int |\hat f|^2\,d\omega$ (equivalently $\gamma(0) = \frac{1}{2\pi}\int S(\omega)\,d\omega$) tying total variance to integrated spectrum, and **periodogram** estimation $\hat S(\omega) = \frac{1}{N}\big|\sum_t x_t e^{-i\omega t}\big|^2$ (with windowing/averaging) computing the spectrum directly from data through the FFT.
...
- **Laplace Convolution Theorem**: the transform of a convolution is the product of transforms,
  $$\mathcal{L}\{(f * g)(t)\} = F(s)\,G(s), \qquad (f * g)(t) = \int_0^t f(\tau)\,g(t-\tau)\,d\tau,$$
  applied in both directions — factoring a product $F(s)G(s)$ so its inverse is a time-domain convolution (rather than partial-fractioning), and collapsing a convolution integral into an algebraic product. This is the engine behind solving **Volterra integral equations** and the **renewal equation** $m = F + m * dF$ in transform space (cross-reference the renewal-theory / stochastic Laplace subsection §7.8.5, where $\tilde m(s) = \tilde f(s)/(1 - \tilde f(s))$).
...
- **Inclusion–exclusion & Bonferroni bounds**: the combinatorial **inclusion–exclusion principle** $\left|\bigcup_i A_i\right| = \sum_i |A_i| - \sum_{i<j} |A_i \cap A_j| + \cdots + (-1)^{n+1}\left|\bigcap_i A_i\right|$ (the counting form reused by §7.18) and its probability form $P\!\left(\bigcup_i E_i\right) = \sum_{k\ge1} (-1)^{k+1}\!\!\sum_{|S|=k} P\!\left(\bigcap_{i\in S} E_i\right)$. Truncating the alternating sum after $k$ terms gives the **Bonferroni inequalities** — the successive partial sums bound $P(\bigcup_i E_i)$ alternately from above (odd $k$) and below (even $k$) — whose first-order case is the **union bound** $P\!\left(\bigcup_i E_i\right) \le \sum_i P(E_i)$ used pervasively in the tail-bound (§7.7.6) and probabilistic-method (§7.18) analyses.
...
#### 7.7.6. Concentration & Tail Inequalities
Analysis tools that bound the probability a random variable deviates from its mean — the backbone of randomized-algorithm (§7.8.6) and probabilistic-method (§7.18) analysis. Each is provided as a symbolic inequality carrying the hypotheses it requires, and the exponential bounds reuse the moment-generating-function machinery of §7.7.4.
- **Markov's inequality**: for $X \ge 0$ and $a>0$, $P(X \ge a) \le \mathbb{E}[X]/a$ — the base first-moment tail bound from which the others are derived.
- **Chebyshev's inequality**: $P(|X-\mu| \ge k\sigma) \le 1/k^2$ (Markov applied to $(X-\mu)^2$), a distribution-free second-moment bound.
- **Chernoff bounds**: the **MGF / exponential-moment method** $P(X \ge a) \le \inf_{t>0} e^{-ta} M_X(t)$, optimized over $t$ and reusing the MGF layer (§7.7.4); specialized in closed form for sums of independent Bernoulli/Poisson variables to give exponentially small tails.
- **Hoeffding's inequality**: for a sum $S=\sum_i X_i$ of independent bounded variables $X_i \in [a_i,b_i]$, $P(|S-\mathbb{E}S| \ge t) \le 2\exp\!\big(-2t^2/\sum_i (b_i-a_i)^2\big)$.
- **Azuma–Hoeffding inequality**: for a martingale (§7.8.2) $\{Y_k\}$ with bounded differences $|Y_k-Y_{k-1}| \le c_k$, $P(|Y_n-Y_0| \ge t) \le 2\exp\!\big(-t^2/(2\sum_k c_k^2)\big)$ — the martingale generalization of Hoeffding.
- **McDiarmid's bounded-differences inequality**: for $f(X_1,\dots,X_n)$ of independent inputs whose value changes by at most $c_i$ when coordinate $i$ alone is altered, $P(|f-\mathbb{E}f| \ge t) \le 2\exp\!\big(-2t^2/\sum_i c_i^2\big)$ (via the Doob martingale and Azuma).
- **Bernstein's inequality**: a variance-aware refinement $P(|S-\mathbb{E}S| \ge t) \le 2\exp\!\big(-\tfrac{t^2/2}{\sum_i \sigma_i^2 + Mt/3}\big)$ for bounded ($|X_i|\le M$) independent summands, sharper than Hoeffding when the total variance $\sum_i \sigma_i^2$ is small.
- **Role**: these give the exponential tail bounds invoked throughout randomized analysis — sample-complexity / estimation-error guarantees for the Monte Carlo estimators and success-probability amplification for randomized algorithms (§7.8.6), and the concentration step of the probabilistic method (§7.18).
...
- **MCMC (the Metropolis–Hastings family)**: the general **Metropolis–Hastings** sampler with acceptance probability $\min\!\big(1,\ \frac{\pi(x')\,q(x\mid x')}{\pi(x)\,q(x'\mid x)}\big)$, and its family of proposals/variants — **random-walk Metropolis**, the **independence sampler**, **Gibbs sampling** (component-wise full-conditional updates, the special case with acceptance $1$), **slice sampling**, the gradient-informed **Metropolis-adjusted Langevin algorithm (MALA)** and **Hamiltonian / Hybrid Monte Carlo (HMC)** with the **No-U-Turn Sampler (NUTS)**, **parallel tempering / replica exchange** for multimodal targets, **reversible-jump MCMC** for trans-dimensional models, and **adaptive Metropolis** proposals — targeting posteriors (§7.7.5) and Boltzmann-type distributions, with convergence diagnostics (Gelman–Rubin $\hat R$, effective sample size, and the autocorrelation of §7.7.2). The accept/reject step reuses the rejection sampler above.
- **Quasi-Monte Carlo**: low-discrepancy sequences (Sobol/Halton) for faster $O((\log N)^d/N)$ convergence on smooth integrands.
- **Las Vegas vs. Monte Carlo algorithms**: the two randomized-algorithm classes, analyzed with the concentration bounds of §7.7.6. **Las Vegas** algorithms are *always correct* but have randomized running time (analyzed by its expectation) — e.g. randomized quicksort/quickselect with $O(n\log n)$ / $O(n)$ expected time; **Monte Carlo** algorithms have deterministic running time but a *bounded error probability*, formalized by the complexity classes **RP / co-RP** (one-sided error), **BPP** (two-sided error bounded below $\tfrac12$), and **ZPP** $= \text{RP} \cap \text{co-RP}$ (the Las Vegas / expected-polynomial-time class). **Probability amplification** by $k$ independent repetitions drives the error to $\le \delta^k$ (majority vote for two-sided error, bounded via Chernoff §7.7.6), trading run time for confidence; the expected-runtime and success-probability analyses reuse the tail inequalities of §7.7.6.
...
#### The Probabilistic Method (Existence Proofs)
Following Alon & Spencer's *The Probabilistic Method*, NimbleCAS carries a toolkit of **non-constructive existence proofs** — showing a combinatorial object with a desired property exists by proving a random construction has it with positive probability — used for combinatorial existence and threshold results, and drawing on the concentration/tail inequalities of §7.7.6.

> **Realized** in [`nimblecas.probmethod`](reference/probmethod.md): the **first-moment/expectation** argument (with the always-true averaging dual), the **union bound**, the **second-moment** method (Chebyshev existence bound), the **Lovász Local Lemma** in both the symmetric form — decided *soundly* over `Q` via a rigorous rational enclosure `e_lo < e < e_hi` of the transcendental constant, giving a three-way `exists`/`condition_fails`/`indeterminate` verdict so a positive certificate is never fabricated — and the asymmetric weighted form, plus concrete **Ramsey lower bounds** `R(k,k) > n` via BigInt first-moment comparison. Every certificate is exact and sound (Rule 32); "not certified" never means "does not exist". The **alteration/deletion**, **Janson/Suen**, and **entropy/martingale-concentration** techniques below remain future work.
- **First-moment / expectation argument**: if $\mathbb{E}[X] < 1$ for a non-negative integer counting variable then $P(X=0)>0$, so a configuration avoiding all "bad" events exists; dually, some outcome attains at least the mean, $\exists\,\omega: X(\omega) \ge \mathbb{E}[X]$. Combined with the union bound (§7.7) this yields Ramsey lower bounds and tournament/colouring results.
- **Second-moment method**: from $\operatorname{Var}(X)$ and Chebyshev (§7.7.6), $P(X=0) \le \operatorname{Var}(X)/\mathbb{E}[X]^2 \to 0$, proving $X>0$ **almost always** — the standard route to sharp **thresholds** for random-graph / random-structure properties.
- **Lovász Local Lemma (LLL)**: existence despite many rare "bad" events with limited dependency — the **symmetric** form (each event has probability $\le p$, depends on $\le d$ others, and $e\,p\,(d+1) \le 1 \Rightarrow$ all avoidable simultaneously) and the **general / asymmetric** form with per-event weights $x_i \in (0,1)$ satisfying $P(A_i) \le x_i \prod_{j \sim i}(1-x_j)$.
- **Alteration / deletion method**: build a random object, then delete the few offending elements and bound the survivors' count by expectation — improving first-moment bounds (independent sets, high-girth high-chromatic-number constructions).
- **Janson inequalities**: exponential bounds on $P(X=0)$ for a sum $X=\sum_i \mathbf{1}_{A_i}$ of monotone (increasing) events, controlled by the dependency sum $\Delta = \sum_{i \sim j} P(A_i \cap A_j)$ — the Janson and generalized (Suen) bounds reaching into the dependent regime beyond the second-moment method.
- **Concentration techniques**: the **entropy** (Shearer / subadditivity) method for counting bounds, and **martingale concentration via Azuma** (§7.7.6, §7.8.2) applied to vertex/edge-exposure Doob martingales for sharp concentration results (e.g. chromatic-number concentration).
...
Symbolic limit evaluation is specified here because it is driven by the same recursive differentiation engine (and feeds the definite-integral bound handling above, the convergence tests of §7.3, and the asymptotics of §7.9). `limit(f, x, c)` evaluates $\lim_{x\to c} f(x)$ for finite $c$, the infinities $\pm\infty$, and **one-sided** limits $\lim_{x\to c^{\pm}}$ (with a `Direction` parameter), returning a `Result<Expr>` that reports non-existence rather than guessing.
- **Direct evaluation & continuity**: substitution (§2.2) is attempted first for points where $f$ is continuous; algebraic pre-simplification (factor/cancel, rationalize a conjugate, common denominator) resolves removable cases before any derivative machinery is invoked.
- **Indeterminate forms**: the engine classifies the seven indeterminate forms $\tfrac{0}{0}$, $\tfrac{\infty}{\infty}$, $0\cdot\infty$, $\infty-\infty$, $1^{\infty}$, $0^{0}$, and $\infty^{0}$, and applies the standard **reductions** to the quotient forms — $0\cdot\infty \to \tfrac{0}{0}$ or $\tfrac{\infty}{\infty}$ by moving a factor into the denominator, $\infty-\infty$ by combining over a common denominator, and the power forms $1^\infty, 0^0, \infty^0$ by taking logarithms $\lim \ln f = \lim g\ln h$ (so $\lim f = e^{\lim g \ln h}$).
- **L'Hôpital's rule**: for the $\tfrac{0}{0}$ and $\tfrac{\infty}{\infty}$ forms, $\lim \frac{f}{g} = \lim \frac{f'}{g'}$ by repeated differentiation of numerator and denominator (reusing §7.19), applied iteratively with a recursion/`order` budget and cycle detection so oscillatory or non-terminating cases fail gracefully rather than looping.
- **Series-expansion limits**: as an alternative (and cross-check) to L'Hôpital, Taylor/Laurent/asymptotic expansion (§7.3) of numerator and denominator to matching leading order resolves the limit from the lowest-order surviving terms — often more robust than repeated differentiation for nested or essential-singularity cases.
- **Squeeze theorem**: bounded$\times$vanishing patterns (e.g. $x\sin\tfrac1x \to 0$) are recognized by bracketing $g(x) \le f(x) \le h(x)$ with $\lim g = \lim h$, closing cases where neither substitution nor L'Hôpital applies.
...
#### Pseudo-Differential & Fractional Operators
A pseudo-differential operator $P = p(x, D)$ is defined by a **symbol** $p(x,\xi)$ acting through the Fourier transform (§7.3), generalizing constant-coefficient differential operators (whose symbol is a polynomial in $\xi$) to non-polynomial and non-integer-order symbols. **Fractional differentiation is exactly the non-integer-order case** — it is the pseudo-differential operator whose symbol is a fractional power of $i\xi$ — so the Fourier-symbol picture and the Gamma-based real-space formulas below are two views of one operator family, built on the Gamma function (§7.1) so the order may be any real (or symbolic) value and collapsing to the ordinary derivative/integral at integer order.
- **Definition**:
  $$(Pu)(x) = \frac{1}{(2\pi)^n}\int_{\mathbb{R}^n} p(x,\xi)\,\hat u(\xi)\,e^{\,i x\cdot\xi}\,d\xi,$$
  reusing the multi-dimensional (separable) Fourier transform of §7.3; a differential operator $\sum_\alpha a_\alpha(x)\,\partial^\alpha$ has symbol $\sum_\alpha a_\alpha(x)\,(i\xi)^\alpha$.
- **Symbol calculus**: composition and adjoint are carried at the symbol level — the composite $PQ$ has symbol $p \# q \sim \sum_\alpha \frac{1}{\alpha!}\,\partial_\xi^\alpha p\; D_x^\alpha q$ and the formal adjoint $P^\ast$ has symbol $\sim \sum_\alpha \frac{1}{\alpha!}\,\partial_\xi^\alpha D_x^\alpha \overline{p}$ (asymptotic expansions), with symbol **order** tracked for ellipticity/parametrix bookkeeping.
- **Weyl quantization**: the symmetric ordering $p^{W}(x,D)$, whose symbol is evaluated at the midpoint $\frac{x+y}{2}$, yielding formally self-adjoint operators for real symbols (the quantization backing the §7.15 quantum operators).
- **Fractional differentiation (symbol $(i\xi)^\alpha$)**: the fractional derivative $D^\alpha$ is the pseudo-differential operator with symbol $(i\xi)^\alpha$, i.e. $\widehat{D^\alpha f}(\xi) = (i\xi)^\alpha\,\hat f(\xi)$ for any real/symbolic order $\alpha$, collapsing to the ordinary $n$-th derivative at $\alpha=n$. The **fractional Laplacian** $(-\Delta)^s$ is the canonical multi-dimensional case, with symbol $|\xi|^{2s}$: $\widehat{(-\Delta)^s u}(\xi) = |\xi|^{2s}\,\hat u(\xi)$ — a non-local operator with an equivalent hypersingular real-space integral on the numerical path.
- **Real-space (Gamma-based) forms** — the equivalent Riemann–Liouville / Caputo definitions, each convolution dispatched to the integration engine above: the **Riemann–Liouville fractional integral** ($\alpha>0$, lower limit $a$) $I_a^\alpha f(x) = \frac{1}{\Gamma(\alpha)}\int_a^x (x-t)^{\alpha-1} f(t)\,dt$ against the power kernel $\frac{(x-t)^{\alpha-1}}{\Gamma(\alpha)}$ (with the **semigroup law** $I_a^\alpha I_a^\beta = I_a^{\alpha+\beta}$ enforced as a simplification identity); the **Riemann–Liouville derivative** $D_a^\alpha f = \frac{d^n}{dx^n}\, I_a^{\,n-\alpha} f$, $n=\lceil\alpha\rceil$; the **Caputo derivative** ${}^{C}\!D_a^\alpha f = I_a^{\,n-\alpha}\,\frac{d^n}{dx^n} f$, which annihilates constants and takes ordinary initial conditions (the form for fractional ODE/IVPs); and the **Grünwald–Letnikov** limit $D^\alpha f(x) = \lim_{h\to 0} h^{-\alpha}\sum_{k\ge 0} (-1)^k \binom{\alpha}{k} f(x-kh)$ with the generalized binomial $\binom{\alpha}{k} = \frac{\Gamma(\alpha+1)}{\Gamma(k+1)\,\Gamma(\alpha-k+1)}$ (§7.1, §7.18) for the numerical path.
- **Closed forms**: the power rule $D^\alpha x^p = \dfrac{\Gamma(p+1)}{\Gamma(p-\alpha+1)}\,x^{p-\alpha}$ (integral companion $I^\alpha x^p = \dfrac{\Gamma(p+1)}{\Gamma(p+\alpha+1)}\,x^{p+\alpha}$), the exponential $D^\alpha e^{\lambda x} = \lambda^\alpha e^{\lambda x}$, and the **Mittag-Leffler** solutions $E_\alpha(\lambda x^\alpha)$ of the linear fractional ODE ${}^{C}\!D^\alpha y = \lambda y$, all recognized symbolically.

#### Operational & Umbral Calculus
The algebra of operators treated as first-class symbolic objects — differentiation, shift, and difference operators manipulated by the same automatic simplification that handles ordinary expressions.
- **Operator algebra**: the differentiation operator $D = \frac{d}{dx}$ and the **shift operator** $E$ ($E f(x) = f(x+1)$, generally $E^h f(x) = f(x+h)$); in the constant-coefficient case they commute as polynomials/power series, so a linear constant-coefficient ODE factors as $\prod_i (D - r_i)\,y = 0$ over the operator ring.
- **Heaviside operational calculus**: solving linear ODEs by treating $D$ **algebraically** — e.g. $(D-a)^{-1} g = e^{ax}\!\int e^{-ax} g\,dx$ and partial-fractioning $1/P(D)$ — rigorously justified by the **Laplace transform** (§7.6), under which $D \leftrightarrow s$ turns the operator algebra into ordinary algebra in $s$.
- **Exponential shift**: the operator identity $e^{aD} f(x) = f(x+a)$ (Taylor's theorem read as $E^a = e^{aD}$), and the shift rule $P(D)\big(e^{ax} g\big) = e^{ax} P(D+a) g$ used to strip exponential factors from ODE right-hand sides.
- **Finite-difference operators**: forward $\Delta = E - I$, backward $\nabla = I - E^{-1}$, and central differences, with the umbral identities $\Delta = e^{D} - I$ (so $D = \ln(I+\Delta)$) and the summation-by-parts operators linking them to the sequence/difference calculus of §7.9.
- **Umbral calculus**: **Sheffer** and **Appell** polynomial sequences defined by a generating function $\sum_n s_n(x)\frac{t^n}{n!} = A(t)\,e^{x B(t)}$, with the lowering/raising operator pair and the umbral substitution $x^n \mapsto a_n$ that recasts identities among the Bernoulli/Euler/Hermite families (§7.16, §7.18) as operator statements.
...
- **Identity / Pattern Filter (fast identity recall)**: the same XOR / Binary Fuse mechanism is generalized into a reusable *identity/pattern filter* backing the engine's identity catalogs — the **trigonometric identities** (§7.1), the **infinite-product identities** and **series identities** (§7.3), and the integration table (§7.19, which already uses this filter). Each catalogued identity's left-hand-side pattern is reduced to a canonical **structural fingerprint** (reusing the engine's structural hashing / hash-consing, §2 and §6.2) and inserted into the filter. At recall time a candidate sub-expression's fingerprint is queried against the filter first — a near-$O(1)$ test with tiny memory that rejects the overwhelming majority of non-matching expressions with **no false negatives** (only rare false positives), so the exact identity hash-table is probed only on a filter hit. This lets the simplification (§2.2) and integration (§7.19) layers apply identity matching **speculatively and cheaply** during a single AST traversal, instead of scanning the full identity tables at every node.
